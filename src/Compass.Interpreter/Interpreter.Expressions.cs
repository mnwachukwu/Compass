...
public sealed partial class Interpreter
{
        ...
        Instance? receiver)
    {
        ...
        return conversion.Operation switch
        {
            ...
            ConversionOperation.StringToCharacters => value is string text
                ? new CompassSet<object?>(text.Select(c => (object?)c))
                : null,

            ConversionOperation.CharactersToString =>
                value is CompassSet<object?> ? CharactersToString(value) : null,
            _ => value,
        };
    }

    private static string CharactersToString(object? value) =>
        value is CompassSet<object?> set
    ...
    private static object? Power(object? left, object? right) => (left, right) switch
    {
        ...
        (long b, long e) => CompassArithmetic.Power(b, e),
        (decimal b, decimal e) => CompassMath.Pow(b, e),
        (double b, double e) => CompassMath.Pow(b, e),
        _ => null,
    };
    ...
    /// means are <see cref="CompassArithmetic"/>'s, so that an emitted program answers them the same
    ...
    {
        BinaryOperator.Add => CompassArithmetic.Add(a, b),
        BinaryOperator.Subtract => CompassArithmetic.Subtract(a, b),
        BinaryOperator.Multiply => CompassArithmetic.Multiply(a, b),
        BinaryOperator.Divide => CompassArithmetic.Divide(a, b),
        BinaryOperator.Remainder => CompassArithmetic.Remainder(a, b),
        BinaryOperator.ShiftLeft => CompassArithmetic.ShiftLeft(a, b),
        BinaryOperator.ShiftRight => CompassArithmetic.ShiftRight(a, b),
    ...
    /// <see cref="CompassArithmetic"/>'s, so that an emitted program stops in the same places and
    ...
    /// since <c>CM0248</c> is what allows a <c>base(...)</c> to be anywhere else.
    /// </summary>
    ...
    /// have meant without saying — and <c>CM0250</c> has already refused the program where the
        ...
        Instance? receiver)
    {
        ...
        {
            return new CompassRuntimeException($"Cannot construct '{construction.TypeName}'.");
        }
        ...
        return built as Exception
            ?? (object)new CompassRuntimeException($"Cannot construct '{construction.TypeName}'.");
    }
    ...
    private object? EvaluateIndex(IndexExpr index, Environment scope, Instance? receiver)
    {
        ...
        {
            CompassSet<object?> set => set[(int)position],
            string text => CompassText.At(text, position),
            _ => null,
        };
    }
