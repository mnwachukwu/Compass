...
public sealed partial class Interpreter
{
    ...
    /// </summary>
    private const string RaisedByProgram = "Compass.RaisedByProgram";
    ...
    /// (<c>CM0405</c>) rather than answered with whatever the cell happened to hold.</para>
    /// </summary>
    ...
    {
        if (Evaluate(walk.Sequence, scope, receiver) is not ICompassSet sequence)
        {
    ...
    /// <para>Compass's exceptions are .NET exceptions, so catching means matching the runtime
    ...
    private ExecutionResult ExecuteTry(TryStmt tryStmt, Environment scope, Instance? receiver)
    {
        ...
        try
        {
            ...
            catch (Exception thrown) when (thrown is not CompassRuntimeException)
            {
                ...
                if (_model.GetSymbol(handler) is { } bound)
                {
                    ...
                    caught.Declare(bound, thrown is CompassThrow custom ? custom.Thrown : thrown);
                }
    ...
    private CatchClause? FindHandler(TryStmt tryStmt, Exception thrown)
    {
        ...
        foreach (CatchClause clause in tryStmt.Catches)
        {
            ...
            bool matches = thrown is CompassThrow custom
    ...
    private ExecutionResult ExecuteThrow(ThrowStmt statement, Environment scope, Instance? receiver)
    {
        ...
        throw value switch
        {
            ...
            Instance instance => new CompassThrow(instance),
            _ => new CompassRuntimeException(Runtime.ModelOperations.ToDisplayString(value)),
        };
    }
        ...
        Instance? receiver)
    {
        ...
        switch (assignment.Target)
        {
            ...
            case IndexExpr index:
            {
                ...
                if (target is Runtime.CompassSet<object?> set)
                {
