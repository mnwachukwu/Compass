...
namespace Compass.Compiler.Lexing;
...
public sealed class Lexer
{
    ...
    /// all in Compass, together with what to write instead.</para>
    /// <para>Every entry here is unambiguous, which is what makes reporting it a scanner's
    /// business rather than a parser's. "++" qualifies because Compass has no unary plus, so
    ...
    /// <para>Several of these have an exact stand-in, since Compass spells the same operation
    ...
    [
        ...
        ("+=", TokenType.Equal, null, "Compass has no compound assignment. Write 'x = x + y'."),
        ("-=", TokenType.Equal, null, "Compass has no compound assignment. Write 'x = x - y'."),
        ("*=", TokenType.Equal, null, "Compass has no compound assignment. Write 'x = x * y'."),
        ("/=", TokenType.Equal, null, "Compass has no compound assignment. Write 'x = x / y'."),
        ("%=", TokenType.Equal, null, "Compass has no compound assignment. Write 'x = x % y'."),
        ...
        ("++", null, null, "Compass has no increment operator. Write 'x = x + 1'."),
        ...
        ("**", TokenType.Caret, "^", "Compass raises to a power with '^'. Write 'base ^ exponent', "
    ...
    /// <para>Sequences that are operators in C# but not here are reported with the Compass
