...
/// <para><c>==</c> in Compass is deep and structural: two models holding equal fields are equal
/// without either of them having written anything to say so. The runtime does that walk, and it
/// reaches a value's parts through <see cref="ICompassModel"/> — three members, because
...
public sealed partial class CilEmitter
{
    ...
    private static readonly MethodInfo TypeIdentityGetter =
        typeof(ICompassModel).GetProperty(nameof(ICompassModel.DeepTypeIdentity))!.GetMethod!;

    private static readonly MethodInfo MemberCountGetter =
        typeof(ICompassModel).GetProperty(nameof(ICompassModel.DeepMemberCount))!.GetMethod!;

    private static readonly MethodInfo GetMemberMethod =
        typeof(ICompassModel).GetMethod(nameof(ICompassModel.GetDeepMember))!;

    /// <summary>
    /// <para>Written as explicit implementations, so that none of the three is reachable by name
    /// from a Compass program and a model declaring a member called <c>DeepMemberCount</c> takes
    /// nothing away.</para>
    /// </summary>
    ...
    private void ImplementDeepEquality(Shaped declaration)
    {
        ...
        type.AddInterfaceImplementation(typeof(ICompassModel));
    ...
    /// <summary>
    /// <para>Which Compass type this is, as the emitted type itself.</para>
    /// <para>Belt and braces here rather than the whole answer: the runtime has already found
    /// the two host types equal by the time it asks, and every Compass type is emitted as a host
    ...
    private static void EmitTypeIdentity(TypeBuilder type)
    {
        MethodBuilder getter = type.DefineMethod(
            "Compass.Runtime.ICompassModel.get_DeepTypeIdentity",
    ...
    {
        MethodBuilder getter = type.DefineMethod(
            "Compass.Runtime.ICompassModel.get_DeepMemberCount",
            AnsweringTheRuntime | MethodAttributes.SpecialName,
            typeof(int),
            Type.EmptyTypes);
    ...
    private static void EmitGetMember(TypeBuilder type, FieldBuilder[] parts)
    {
        MethodBuilder method = type.DefineMethod(
            "Compass.Runtime.ICompassModel.GetDeepMember",
            AnsweringTheRuntime,
            typeof(object),
            [typeof(int)]);
