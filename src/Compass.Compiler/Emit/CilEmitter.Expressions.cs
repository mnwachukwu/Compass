...
public sealed partial class CilEmitter
{
    ...
    /// value is there has been settled by <c>CM0401</c> long before.</para>
    /// </summary>
    ...
    private void EmitBinary(BinaryExpr binary)
    {
        ...
        // call into the runtime rather than one opcode. See CompassArithmetic: emitted as 'add'
    ...
    /// would call <c>ToString</c> and put <c>True</c> in a Compass string. How a value reads is
    ...
    {
        BinaryOperator.Add => Arithmetic(nameof(CompassArithmetic.Add)),
        BinaryOperator.Subtract => Arithmetic(nameof(CompassArithmetic.Subtract)),
        BinaryOperator.Multiply => Arithmetic(nameof(CompassArithmetic.Multiply)),
        BinaryOperator.Divide => Arithmetic(nameof(CompassArithmetic.Divide)),
        BinaryOperator.Remainder => Arithmetic(nameof(CompassArithmetic.Remainder)),
        BinaryOperator.ShiftLeft => Arithmetic(nameof(CompassArithmetic.ShiftLeft)),
        BinaryOperator.ShiftRight => Arithmetic(nameof(CompassArithmetic.ShiftRight)),
        _ => null,
    };

    private static MethodInfo Arithmetic(string name) =>
        typeof(CompassArithmetic).GetMethod(name, [typeof(long), typeof(long)])!;
    ...
    {
        BinaryOperator.Add => OnReals(nameof(CompassArithmetic.Add)),
        BinaryOperator.Subtract => OnReals(nameof(CompassArithmetic.Subtract)),
        BinaryOperator.Multiply => OnReals(nameof(CompassArithmetic.Multiply)),
        BinaryOperator.Divide => OnReals(nameof(CompassArithmetic.Divide)),
        BinaryOperator.Remainder => OnReals(nameof(CompassArithmetic.Remainder)),
    ...
    private static MethodInfo OnReals(string name) =>
        typeof(CompassArithmetic).GetMethod(name, [typeof(decimal), typeof(decimal)])!;
    ...
    private void EmitConversionStep(ConversionOperation operation)
    {
        switch (operation)
        {
            ...
            // whole numbers a fraction is made of, which is CM0346 where it is written down and
    ...
    private static readonly MethodInfo FormatInteger =
        typeof(CompassText).GetMethod(
            nameof(CompassText.Format), [typeof(long), typeof(string)])!;

    private static readonly MethodInfo FormatReal =
        typeof(CompassText).GetMethod(
            nameof(CompassText.Format), [typeof(decimal), typeof(string)])!;

    private static readonly MethodInfo FormatFloat =
        typeof(CompassText).GetMethod(
            nameof(CompassText.Format), [typeof(double), typeof(string)])!;
    ...
    private static readonly MethodInfo RealAsFloat =
        typeof(CompassArithmetic).GetMethod(
            nameof(CompassArithmetic.ToFloat), [typeof(decimal)])!;

    private static readonly MethodInfo FloatAsReal =
        typeof(CompassArithmetic).GetMethod(
            nameof(CompassArithmetic.ToReal), [typeof(double)])!;

    private static readonly MethodInfo WriteOfObject =
        typeof(CompassConsole).GetMethod(nameof(CompassConsole.Write), [typeof(object)])!;

    private static readonly MethodInfo WriteLineOfObject =
        typeof(CompassConsole).GetMethod(nameof(CompassConsole.WriteLine), [typeof(object)])!;

    private static readonly MethodInfo WriteLineOfNothing =
        typeof(CompassConsole).GetMethod(nameof(CompassConsole.WriteLine), Type.EmptyTypes)!;
    ...
    private static readonly MethodInfo ReadLine =
        typeof(CompassConsole).GetMethod(nameof(CompassConsole.Read), Type.EmptyTypes)!;
