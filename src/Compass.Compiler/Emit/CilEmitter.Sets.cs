...
/// <summary>
/// <para>Sets, which become <see cref="Runtime.CompassSet{T}"/> — the very type the interpreter
/// holds.</para>
...
/// and removing are part of a Compass set's surface and an array's length is fixed.</para>
/// <para><b>Positions are converted at this boundary.</b> Compass counts with <c>integer</c>,
...
public sealed partial class CilEmitter
{
    ...
    /// <summary>
    /// An index, narrowed to what the CLR indexes with. Compass counts in 64 bits and a list
    ...
    {
        MethodInfo definition = typeof(Runtime.CompassSet<>)
    ...
    /// <para>The set is <c>CompassSet&lt;Optional&lt;V&gt;&gt;</c> and the answer is a
    /// <c>CompassSet&lt;V&gt;</c>, so what is needed is the <c>V</c> — one level in past the
    ...
    private static MethodInfo WithoutEmpties(Type built)
    {
        ...
        return typeof(Runtime.CompassSet)
            .GetMethod(nameof(Runtime.CompassSet.WithoutEmpties))!
    ...
    {
        ConstructorInfo definition = typeof(Runtime.CompassSet<>)
