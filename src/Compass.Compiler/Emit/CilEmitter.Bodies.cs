...
public sealed partial class CilEmitter
{
    ...
    private void EmitBody(Declared declared)
    {
        ...
        // left down every path — proved by CM0404 rather than assumed. Either way the method
    ...
    /// and will not verify one without. <c>CM0248</c> is what makes that possible to honor while
    ...
    /// first statement is looked at, since <c>CM0248</c> is what put it there.
    /// </summary>
    ...
    /// <c>CM0250</c> has already established that the parent can be built with nothing — so the
    /// call is to whichever constructor takes no arguments. Where the parent is not a model this
    /// program declared, the chain ends at <c>System.Object</c>, which is what Compass's
    /// <c>Model</c> is.</para>
    /// </summary>
