...
/// <summary>
/// <para>What each Compass type is, once it reaches the CLR.</para>
...
internal static class CilTypes
{
    ...
    /// <c>System.IO.IOException</c>, and a Compass <c>catch</c> becomes a CIL one.</para>
    ...
    /// are part of a Compass set's surface and an array's length is fixed. Sharing it is also
    /// what keeps the two engines agreeing about what <c>Remove</c> does to the order.</para>
    /// </summary>
    public static Type SetOf(Type element) =>
        typeof(Runtime.CompassSet<>).MakeGenericType(element);
    ...
    /// <c>string?</c> is the reference itself. Compass has no null to reuse for the second case,
    ...
    /// <c>DateOnly</c>, a time of day a <c>TimeOnly</c> — so a Compass moment handed to .NET is a
    ...
    /// <see cref="Runtime.CompassMoments"/> refuse an impossible date in the language's words, and
    ...
    public static Type? OfProvided(TypeSymbol type) =>
        type is ModelSymbol model && ReferenceEquals(model.Container, BuiltInTypes.Standard)
            ? model.Name switch
            {
                ...
                "Random" => typeof(Runtime.CompassRandom),
                _ => null,
            }
            : null;
    ...
    /// <summary>
    /// The CLR type a function gives back. Null in Compass means it yields nothing, which is
