...
/// <param name="Id">Set when it named an identifier such as <c>CM0340</c>.</param>
...
public static class SuppressionDirective
{
        ...
        out string? id)
    {
        ...
        if (word.Length == 6
            && char.ToUpperInvariant(word[0]) == 'C'
            && char.ToUpperInvariant(word[1]) == 'M'
            && word.AsSpan(2).ContainsAnyExceptInRange('0', '9') is false)
        {
            id = string.Concat("CM", word.AsSpan(2));
            return true;
        }
