...
/// </summary>
public static class CompassFiles
{
    ...
    public static Optional<CompassSet<string>> ReadLines(string path) =>
        File.Exists(path)
            ? Optional<CompassSet<string>>.Of(new CompassSet<string>(File.ReadAllLines(path, Utf8)))
            : default;
    ...
    /// one.</para>
    /// </summary>
    public static void WriteLines(string path, ICompassSet lines)
    {
    ...
    public static Optional<CompassSet<string>> Files(string path) =>
        Directory.Exists(path)
            ? Optional<CompassSet<string>>.Of(new CompassSet<string>(Sorted(Directory.GetFiles(path))))
            : default;

    public static Optional<CompassSet<string>> Folders(string path) =>
        Directory.Exists(path)
            ? Optional<CompassSet<string>>.Of(
                new CompassSet<string>(Sorted(Directory.GetDirectories(path))))
            : default;
    ...
    /// <para><inheritdoc cref="CompassText.ToCharactersUntyped" path="/summary/para[2]"/></para>
    /// </summary>
    ...
    private static CompassSet<object?> Untyped(IEnumerable<string> lines) =>
