...
/// <para>Compass's <c>==</c> compares models and sets by their contents rather than by
...
public static class DeepEquality
{
        ...
        Stack<(object Left, object Right)> pending)
    {
        ...
        switch (a)
        {
            ...
            case ICompassOptional optionalA:
            {
                ICompassOptional optionalB = (ICompassOptional)b;
            ...
            case ICompassSet setA:
            {
                ICompassSet setB = (ICompassSet)b;
            ...
            case ICompassModel modelA:
            {
                ICompassModel modelB = (ICompassModel)b;

                // Asked of the value rather than of its host type: the interpreter runs every
                // model and structure as one class, so the check above cannot tell two Compass
                // types apart.
