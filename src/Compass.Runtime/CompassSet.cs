...
/// <summary>
/// <para>Compass's set: an ordered, indexed, growable sequence that permits duplicates.</para>
...
/// </summary>
public sealed class CompassSet<T> : ICompassSet, IEnumerable<T>
{
    private readonly List<T> _items;

    /// <summary>Creates an empty set.</summary>
    public CompassSet() => _items = [];

    /// <summary>Creates a set from a literal's elements.</summary>
    public CompassSet(IEnumerable<T> items)
    {
        ArgumentNullException.ThrowIfNull(items);
        _items = [.. items];
    }

    /// <summary>How many elements the set holds. A method in Compass, since it has no properties.</summary>
    ...
    public CompassSet<T> Subset(int start, int end)
    {
        ...
        return new CompassSet<T>(_items.Skip(start).Take(end - start));
    }

    /// <summary>A run from <paramref name="start"/> to the end.</summary>
    public CompassSet<T> Subset(int start) => Subset(start, _items.Count);

    /// <summary>
    /// <para>This set followed by another, end to end.</para>
    /// <para><b>Appends rather than merges.</b> A Compass set keeps its order and allows a value
    ...
    /// </summary>
    public CompassSet<T> Union(CompassSet<T> other)
    {
        ArgumentNullException.ThrowIfNull(other);

        return new CompassSet<T>(_items.Concat(other._items));
    }

    /// <summary>What is in both, in this set's order.</summary>
    public CompassSet<T> Intersect(CompassSet<T> other)
    {
        ArgumentNullException.ThrowIfNull(other);

        return new CompassSet<T>(_items.Where(other.Contains));
    }
    ...
    /// </summary>
    public CompassSet<T> Except(CompassSet<T> other)
    {
        ArgumentNullException.ThrowIfNull(other);

        return new CompassSet<T>(_items.Where(element => !other.Contains(element)));
    }
    ...
    /// count as one value. This is what turns a Compass set into the set of mathematics, which it
    /// is not until asked.</para>
    /// </summary>
    public CompassSet<T> Distinct()
    {
        CompassSet<T> kept = new();
    ...
    private static bool Absent(T element) =>
        element is null || (element is ICompassOptional optional && !optional.HasValue);
    ...
    /// </summary>
    public CompassSet<T> Trim() => Trimmed(start: true, end: true);

    /// <summary>This set with the empties dropped from the front.</summary>
    public CompassSet<T> TrimStart() => Trimmed(start: true, end: false);

    /// <summary>This set with the empties dropped from the back.</summary>
    public CompassSet<T> TrimEnd() => Trimmed(start: false, end: true);
    ...
    /// is <see cref="CompassSet.WithoutEmpties{TValue}"/> that makes it — in the untyped engine
    /// there is nothing to unwrap, so this is the whole of it.</para>
    /// </summary>
    public CompassSet<T> TrimAll() => new(_items.Where(element => !Absent(element)));

    private CompassSet<T> Trimmed(bool start, bool end)
    {
        int first = 0;
    ...
    object? ICompassSet.GetElement(int index) => _items[index];
...
/// </summary>
public static class CompassSet
{
    /// <summary>
    ...
    /// <para><see cref="CompassSet{T}.TrimAll"/> is the same idea where there is nothing to
    /// unwrap, which is how the interpreter holds one.</para>
    /// </summary>
    public static CompassSet<TValue> WithoutEmpties<TValue>(CompassSet<Optional<TValue>> source)
    {
        ArgumentNullException.ThrowIfNull(source);

        CompassSet<TValue> kept = new();
