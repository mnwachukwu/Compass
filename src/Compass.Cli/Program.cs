...
using Compass.Compiler.Formatting;
...
/// <summary>
/// <para>Entry point for the <c>compass</c> command, and for the <c>cm</c> alias that shares
/// it.</para>
...
public static class Program
{
    ...
    [
        new("run", "<file>", "Run a .cm program or a .cmp project", RunProgram,
        [
        ...
        [
            ...
            "configuration, the Compass runtime, and a launcher you can start without",
        ...
        new("check", "<file>", "Check a .cm program or a .cmp project", RunCheck, []),
        new("new", "<name>", "Start an empty program, ready to be written", RunNew,
        [
            "  --project            Write a folder with a .cmp and a program in it",
            "",
        ...
        new("tokens", "<file>", "Scan one .cm file and print its token stream", RunTokens, []),
        new("ast", "<file>", "Parse one .cm file and print its syntax tree", RunAst,
        [
            "  --positions          Write each node's span beside it",
        ]),
        new("format", "<file>", "Line one .cm file up and print it", RunFormat,
        [
        ...
        new("outline", "<file>", "Print what one .cm file declares, as JSON", RunOutline, []),
        new("project", "<file>", "Print the .cmp that builds a file, as JSON", RunProject, []),
    ...
    /// <summary>Runs the command. Public so that the <c>cm</c> alias can forward to it.</summary>
    ...
    private static void WriteUsage()
    {
        ...
        Console.WriteLine($"{ToolName} - the Compass compiler");
        Console.WriteLine();
        ...
        Console.WriteLine();
        Console.WriteLine("Naming a .cm file compiles it together with the shared code beside");
        Console.WriteLine("it: every other .cm in the same folder that declares no Program.");
        Console.WriteLine("A .cmp project compiles exactly what it lists, across any folders.");
        Console.WriteLine();
        Console.WriteLine("The extension may be left off: 'run Program' finds Program.cm or");
        Console.WriteLine("Program.cmp. Write it when both are there and you mean one.");
    }
    ...
    /// <c>build --help</c> answers "file not found: --help.cm" — which names a file nobody meant,
    ...
    /// build rather than being Compass, so there is nothing in one for them to show.
    /// </summary>
    ...
    /// <summary>
    /// <para>Answers an editor's questions about Compass for as long as it stays open, speaking
    ...
    /// half of it is not valid Compass at any given moment — so questions about it can only be
    ...
    /// <c>.cmp</c> is read — and a second reader of that format would agree with this one until
    ...
    private static int RunProgram(string[] args)
    {
        ...
        try
        {
            ...
            return Compass.Interpreter.Interpreter.Run(
                Lowering.Lower(compilation.Units, model), model);
        }
