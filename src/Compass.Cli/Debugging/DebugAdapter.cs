...
namespace Compass.Cli.Debugging;

/// <summary>
/// <para>Speaks the Debug Adapter Protocol on behalf of a Compass program.</para>
...
{
    /// <summary>Compass runs one program on one thread, so there is one to report.</summary>
    ...
    /// <para>Compiled the same way <c>cm run</c> compiles it, down to the file discovery, so
    ...
    /// <para>Each carries where it is, in the same form every other Compass diagnostic is
    ...
    /// adapter has no expressions to evaluate — there is no place in Compass where a reader writes
    ...
    private sealed class Asking(DapConnection wire, Reporting printed) : TextReader
    {
        ...
        public override string? ReadLine()
        {
            ...
            Waiting = true;
            wire.Event("compass/read", new JsonObject { ["prompt"] = printed.Pending });
