...
public static class SourceDiscovery
{
    ...
    /// <summary>The extension of a Compass source file.</summary>
    public const string SourceExtension = ".cm";

    /// <summary>The extension of a Compass project file.</summary>
    ...
    /// <para>An extension may be left off: <c>Program</c> finds <c>Program.cm</c> or
    /// <c>Program.cmp</c>, whichever is there. Writing one keeps its exactness, which is the
    ...
    /// Compass because it says so, not because something tried.</para>
    /// </summary>
    public static FileTarget? Locate(string written, out string problem)
    {
        ...
        {
            // Recognized the same way the file system matches "*.cm" when a folder is read, so
            ...
            if (!project && !source)
            {
                problem = "Not a valid Compass source or project file.";
                return null;
            }
    ...
    /// other <c>.cm</c> in the same folder that declares no <c>Program</c>. A file that does
    ...
    /// <para>Asked the same way <c>cm project</c> answers it, so what the editor is told owns a
    ...
    /// <summary>
    /// Reads a circle back as a sentence: <c>A.cm imports B.cm, which imports A.cm</c>. Files
        ...
        DiagnosticBag diagnostics)
    {
        ...
        // file carry this name, and "lib/../other/B.cm" is a worse answer to where than "other/B.cm".
