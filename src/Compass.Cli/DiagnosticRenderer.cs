...
public static class DiagnosticRenderer
{
    ...
    /// <see cref="CompassFailure"/>'s, because an emitted program has to say the same thing at its
    ...
    public static string? DescribeFailure(string label, Exception failure)
    {
        ...
        {
            CompassRuntimeException => $"{label}: {failure.Message}",
            ...
            UncaughtCompassException uncaught =>
                CompassFailure.Describe(label, uncaught.TypeName, uncaught.Text),

            _ => CompassFailure.Describe(label, failure),
        };
    }
