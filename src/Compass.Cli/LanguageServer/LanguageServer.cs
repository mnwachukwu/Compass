...
namespace Compass.Cli.LanguageServer;

/// <summary>
/// <para>Answers an editor's questions about Compass, for as long as the editor is open.</para>
...
/// written. A file mid-edit is not valid Compass and has no version on disk to stand in for it.
...
public sealed class LanguageServer : IDisposable
{
    ...
    private JsonArray? FixesFor(JsonObject? parameters)
    {
        ...
        _ = new Compass.Compiler.Lexing.Lexer(source, found).Scan();
    ...
    /// <para>The same placement <c>cm format</c> would give it, from the same code, which is the
