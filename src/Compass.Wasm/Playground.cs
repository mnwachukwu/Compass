...
public static partial class Playground
{
    ...
    private const string FileName = "playground.cm";
    ...
    /// <para>Given an id of its own rather than a <c>CM</c> number, because it is not one. Nothing
        ...
        foreach (SyntaxNode node in unit.Descendants())
        {
            ...
            JsonObject one = new()
            {
                ...
                ["message"] =
                    ...
                    + "program is fine — run it on your own machine with 'cm run' and this works.",
                ["line"] = start.Line - 1,
    ...
    /// <summary>
    /// <para>The program, laid out the way <c>cm format</c> lays one out.</para>
    ...
    /// with them rather than less: the hover carries markdown holding a line of Compass, and the
    ...
    /// reason <c>cm --version</c> cuts it: that belongs in a build log.</para>
    /// </summary>
    [JSExport]
    ...
    private static string Failure(Exception failed) =>
        failed is Compass.Interpreter.UncaughtCompassException
                  or Compass.Interpreter.CompassRuntimeException
            ? failed.Message
