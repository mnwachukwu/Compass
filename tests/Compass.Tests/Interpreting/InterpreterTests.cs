...
public sealed class InterpreterTests
{
    ...
    /// </para>
    /// <para><c>CM0243</c> catches the name the loop names, and stops at the first thing it
    ...
    public void ACatchDoesNotTakeAFailureTheProgramDidNotCause()
    {
        ...
        Assert.Throws<NotSupportedException>(() => Compass.Interpreter.Interpreter.Run(
    ...
    public void AnUncaughtBareExceptionIsDescribedRatherThanDumped() => Assert.That(
        Assert.Throws<Compass.Interpreter.UncaughtCompassException>(() => RunBody("""
    ...
    /// the compiler can fold is one it reports (<c>CM0414</c>) before this ever runs. Written as
    ...
    {
        string path = Path.Combine(LexerTestBase.RepositoryRootForTests, "samples", "hello.cm");
    ...
    public void AValueIsOfItsOwnTypeAndTheAnswerIsSettledWhileCompiling()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(ids, Is.EqualTo(new[] { "CM0334", "CM0334", "CM0334" }));
        });
    }
