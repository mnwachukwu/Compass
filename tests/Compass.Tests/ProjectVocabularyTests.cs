...
/// <see cref="Compass.Cli.ProjectFile"/> reads them as cases in a switch, so a word added to the
...
{
    /// <summary>
    /// <para>The whole vocabulary, as <see cref="Compass.Cli.ProjectFile"/> reads it.</para>
    ...
    private static IEnumerable<string> EveryProjectLine =>
        Directory.EnumerateFiles(
                     ...
                     "*" + Compass.Cli.SourceDiscovery.ProjectExtension,
    ...
    /// <c>ambiguous-entry.cmp</c> names the fix it does not apply — so counting the word anywhere
    ...
    /// <c>Program</c> is <c>CM0236</c>, an opinion, and a positive sample may report nothing.
    ...
    /// <c>ambiguous-entry.cmp</c>, which is that project with the line left out.</para>
    /// </summary>
    [Test]
