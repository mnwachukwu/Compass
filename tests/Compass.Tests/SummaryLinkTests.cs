...
/// <summary>
/// <para>The prose documents' own links, and the Compass written in them.</para>
...
{
    /// <summary>The prose documents, each of which carries Compass and links to itself.</summary>
    ...
    /// <summary>
    /// <para>Every Compass fence scans without a lexical diagnostic.</para>
    ...
    /// </summary>
    [TestCaseSource(nameof(Documents))]
    public void EveryCompassSnippetScansCleanly(string document)
    {
        List<string> complaints = [];

        foreach ((string snippet, int at) in CompassFences(document))
        {
        ...
        Assert.That(complaints, Is.Empty, $"Compass in {document} that does not scan");
    }
    ...
    /// <summary>
    /// <para>Every fenced block that is Compass rather than C#, with the line it opens on.</para>
    /// <para>Told apart by the fence's language tag: the C# blocks carry <c>csharp</c> and the
    /// Compass ones carry nothing, since no highlighter knows the language yet.</para>
    /// </summary>
    private static IEnumerable<(string Snippet, int At)> CompassFences(string document)
    {
        List<string> current = [];
        bool inside = false;
        bool compass = false;
