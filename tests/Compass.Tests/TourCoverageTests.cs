...
/// <para><c>samples/reference/tour.cm</c> says every construct in the grammar appears in it at
...
/// <c>bits.cm</c> alone, <c>import</c> in <c>toolkit</c> alone, and two of the six loop forms
/// were missing from the file whose job is to hold all of them.</para>
/// <para>Measured on <c>tour.cm</c> by itself, since that is what the sentence claims. The
...
public sealed class TourCoverageTests : LexerTestBase
{
    ...
    {
        SourceText source = LoadSample("tour.cm");
    ...
    public void TheTourWritesBlockNamespacesAndNotTheFileScopedForm()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(declared.Any(one => one.IsFileScoped), Is.False,
                        ...
                        + "one; namespaces.cm is where that form is written");
        });
    }
}
