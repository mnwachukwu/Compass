...
/// </summary>
[TestFixture]
public sealed class CompassArithmeticTests
{
    /// <summary>
    ...
    public void ArithmeticPastTheEndOfAnIntegerStops()
    {
        Assert.Multiple(() =>
        {
            Assert.Throws<OverflowException>(() => CompassArithmetic.Add(long.MaxValue, 1));
            Assert.Throws<OverflowException>(() => CompassArithmetic.Subtract(long.MinValue, 1));
            Assert.Throws<OverflowException>(() => CompassArithmetic.Multiply(long.MaxValue, 2));
            ...
            Assert.Throws<OverflowException>(() => CompassArithmetic.Divide(long.MinValue, -1));
        });
    }
    ...
    public void OrdinaryArithmeticIsOrdinary()
    {
        Assert.Multiple(() =>
        {
            Assert.That(CompassArithmetic.Add(2, 3), Is.EqualTo(5));
            Assert.That(CompassArithmetic.Subtract(2, 3), Is.EqualTo(-1));
            Assert.That(CompassArithmetic.Multiply(6, 7), Is.EqualTo(42));

            // Division truncates, which is worth knowing: one divided by three is zero.
            Assert.That(CompassArithmetic.Divide(7, 2), Is.EqualTo(3));
            Assert.That(CompassArithmetic.Divide(1, 3), Is.Zero);
            Assert.That(CompassArithmetic.Divide(-7, 2), Is.EqualTo(-3));
            Assert.That(CompassArithmetic.Remainder(7, 3), Is.EqualTo(1));
            ...
            Assert.That(CompassArithmetic.Remainder(long.MinValue, -1), Is.Zero);
        });
    }

    /// <summary>
    /// Dividing by zero is refused in the language's own words, which name <c>CM0324</c> and say
    ...
    public void DividingByZeroIsRefusedInTheLanguagesWords()
    {
        ...
        {
            Assert.That(
                Assert.Throws<DivideByZeroException>(() => CompassArithmetic.Divide(1, 0))!.Message,
                Does.Contain("CM0324"));

            Assert.That(
                Assert.Throws<DivideByZeroException>(() => CompassArithmetic.Remainder(1, 0))!.Message,
    ...
    public void AShiftOutsideTheWidthIsRefusedRatherThanFolded()
    {
        Assert.Multiple(() =>
        {
            Assert.Throws<ArgumentException>(() => CompassArithmetic.ShiftLeft(1, 64));
            Assert.Throws<ArgumentException>(() => CompassArithmetic.ShiftRight(1, 64));
            Assert.Throws<ArgumentException>(() => CompassArithmetic.ShiftLeft(1, -1));

            // Both ends of what an integer does have.
            Assert.That(CompassArithmetic.ShiftLeft(1, 0), Is.EqualTo(1));
            Assert.That(CompassArithmetic.ShiftLeft(1, 10), Is.EqualTo(1024));
            Assert.That(CompassArithmetic.ShiftLeft(1, 63), Is.EqualTo(long.MinValue));
            Assert.That(CompassArithmetic.ShiftRight(1024, 3), Is.EqualTo(128));
        });
    }
}
