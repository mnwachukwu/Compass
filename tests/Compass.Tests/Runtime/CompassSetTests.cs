...
[TestFixture]
public sealed class CompassSetTests
{
    private static CompassSet<int> Set(params int[] items) => new(items);

    [Test]
    public void ANewSetIsEmpty()
    {
        Assert.That(new CompassSet<int>().Count, Is.Zero);
    }

    [Test]
    public void InsertAppends()
    {
        CompassSet<int> set = Set(1, 2);
        set.Insert(3);
    ...
    {
        CompassSet<int> set = Set(1, 2, 2, 3);
        set.Remove(2);
    ...
    {
        CompassSet<int> set = Set(10, 20, 30);
    ...
    {
        CompassSet<CompassSet<int>> nested = new([Set(1, 2), Set(3)]);
    ...
    {
        CompassSet<int> original = Set(1, 2);
        CompassSet<int> alias = original;
    ...
    public void PrintingShowsTheElementsInBraces()
    {
        ...
        Assert.That(new CompassSet<int>().ToString(), Is.EqualTo("{}"));
    }
    ...
    public void PrintingQuotesWhateverCouldBeConfusedWithTheDelimiter()
    {
        ...
        {
            Assert.That(
                new CompassSet<object?>(["a, b"]).ToString(),
                Is.EqualTo("{\"a, b\"}"));
            ...
            Assert.That(
                new CompassSet<object?>([',', ' ']).ToString(),
                Is.EqualTo("{',', ' '}"));

            Assert.That(
                new CompassSet<object?>([true, 1L, null]).ToString(),
    ...
    {
        CompassSet<Optional<long>> sparse = new(
        [
            Optional<long>.Empty,
            Optional<long>.Of(1),
            Optional<long>.Empty,
    ...
    {
        CompassSet<object?> sparse = new([null, 1L, null, 2L, null]);
    ...
    public void WithoutEmptiesDropsThemAllAndUnwrapsTheRest()
    {
        ...
        CompassSet<long> kept = CompassSet.WithoutEmpties(sparse);
    ...
    {
        CompassSet<object?> full = new([1L, 2L, 3L]);
    ...
    {
        CompassSet<object?> empties = new([null, null, null]);
