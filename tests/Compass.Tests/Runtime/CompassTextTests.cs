...
/// <summary>
/// <para>What a Compass <c>string</c> does, tested where it lives rather than through either
/// engine.</para>
...
/// </summary>
[TestFixture]
public sealed class CompassTextTests
{
    /// <summary>
    ...
    public void AnEmptyArgumentChangesNothing()
    {
        Assert.Multiple(() =>
        {
            Assert.That(CompassText.Replace("abc", "", "-"), Is.EqualTo("abc"));
            Assert.That(CompassText.Remove("abc", ""), Is.EqualTo("abc"));
            Assert.That(CompassText.Trim("abc", ""), Is.EqualTo("abc"));
            Assert.That(CompassText.TrimStart("abc", ""), Is.EqualTo("abc"));
            Assert.That(CompassText.TrimEnd("abc", ""), Is.EqualTo("abc"));
        });
    }
    ...
    public void AnEmptyArgumentIsFoundWhereverItIsLookedFor()
    {
        Assert.Multiple(() =>
        {
            Assert.That(CompassText.Contains("abc", ""), Is.True);
            Assert.That(CompassText.IndexOf("abc", ""), Is.Zero);
        });
    }
    ...
    public void SeparatingOnNothingLeavesOnePiece()
    {
        Assert.Multiple(() =>
        {
            Assert.That(CompassText.Split("a,b", "").Count, Is.EqualTo(1));
            Assert.That(CompassText.Split("a,b", "")[0], Is.EqualTo("a,b"));
            Assert.That(CompassText.SplitUntyped("a,b", "").Count, Is.EqualTo(1));
        });
    }
    ...
    {
        CompassSet<char> typed = CompassText.ToCharacters("abc");
        CompassSet<object?> untyped = CompassText.ToCharactersUntyped("abc");
    ...
    {
        CompassSet<char> typed = new(['x', 'y']);
        CompassSet<object?> untyped = new(['x', 'y']);

        Assert.Multiple(() =>
        {
            Assert.That(CompassText.Trim("xyabcyx", typed), Is.EqualTo("abc"));
            Assert.That(CompassText.Trim("xyabcyx", untyped), Is.EqualTo("abc"));
        });
    }
    ...
    public void CapitalizingLeavesTheRestAlone()
    {
        Assert.Multiple(() =>
        {
            Assert.That(CompassText.Capitalize("mcDonald"), Is.EqualTo("McDonald"));
            Assert.That(CompassText.Capitalize(""), Is.Empty);
        });
    }
    ...
    public void APositionOutsideTheStringIsRefusedInTheLanguagesWords()
    {
        ...
        {
            Assert.That(
                Assert.Throws<IndexOutOfRangeException>(() => CompassText.RemoveAt("abc", 3))!.Message,
            ...
            // is a thing to mean.
            Assert.That(CompassText.InsertAt("abc", 3, "d"), Is.EqualTo("abcd"));
        });
    }
    ...
    public void TextReadsBackAsANumberOrAsNothing()
    {
        Assert.Multiple(() =>
        {
            Assert.That(CompassText.ToInteger("42").Value, Is.EqualTo(42L));
            Assert.That(CompassText.ToInteger("four").HasValue, Is.False);
            Assert.That(CompassText.ToReal("3.5").Value, Is.EqualTo(3.5));
            Assert.That(CompassText.ToBoolean(" true ").Value, Is.True);

            // Only the two words the language writes, so these are not truths.
            Assert.That(CompassText.ToBoolean("yes").HasValue, Is.False);
            Assert.That(CompassText.ToBoolean("1").HasValue, Is.False);
        });
    }
    ...
    public void ANumberWritesByItsPatternWhereverItRuns()
    {
        Assert.Multiple(() =>
        {
            Assert.That(CompassText.Format(1234L, "N0"), Is.EqualTo("1,234"));
            Assert.That(CompassText.Format(3.14159, "F2"), Is.EqualTo("3.14"));
        });
    }
}
