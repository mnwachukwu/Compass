...
/// </summary>
[TestFixture]
public sealed class CompassMomentsTests
{
    /// <summary>
    ...
    {
        DateTime moment = CompassMoments.MakeMoment(1969, 7, 20, 20, 17, 40);

        Assert.Multiple(() =>
        {
            Assert.That(CompassMoments.Year(moment), Is.EqualTo(1969L));
            Assert.That(CompassMoments.Month(moment), Is.EqualTo(7L));
            Assert.That(CompassMoments.Day(moment), Is.EqualTo(20L));
            Assert.That(CompassMoments.Hour(moment), Is.EqualTo(20L));
            Assert.That(CompassMoments.Minute(moment), Is.EqualTo(17L));
            Assert.That(CompassMoments.Second(moment), Is.EqualTo(40L));

            // Sunday is zero, which the sample says out loud because nothing else would.
            Assert.That(CompassMoments.DayOfWeek(moment), Is.EqualTo(0L));
        });
    }
        ...
        Assert.Multiple(() =>
        {
            Assert.That(
                Assert.Throws<ArgumentException>(
                    () => CompassMoments.MakeMoment(2026, 2, 31, 0, 0, 0))!.Message,
                Is.EqualTo("There is no such moment as 2026-2-31."));

            Assert.That(
                Assert.Throws<ArgumentException>(() => CompassMoments.MakeDate(2026, 13, 1))!.Message,
                Is.EqualTo("There is no such date as 2026-13-1."));

            Assert.That(
                Assert.Throws<ArgumentException>(() => CompassMoments.MakeTime(25, 0, 0))!.Message,
    ...
    {
        TimeSpan length = CompassMoments.MakeSpan(1, 2, 30, 0);

        Assert.Multiple(() =>
        {
            Assert.That(CompassMoments.Days(length), Is.EqualTo(1L));
            Assert.That(CompassMoments.Hours(length), Is.EqualTo(2L));
            Assert.That(CompassMoments.Minutes(length), Is.EqualTo(30L));

            Assert.That(CompassMoments.TotalHours(length), Is.EqualTo(26.5m));
            ...
            Assert.That(CompassMoments.TotalDays(length), Is.EqualTo(1.10416666666667m));
        });
    }
    ...
    {
        TimeSpan length = CompassMoments.MakeSpan(26, 0, 0);

        Assert.Multiple(() =>
        {
            Assert.That(CompassMoments.Days(length), Is.EqualTo(1L));
            Assert.That(CompassMoments.Hours(length), Is.EqualTo(2L));
            Assert.That(CompassMoments.TotalHours(length), Is.EqualTo(26m));
        });
    }
    ...
    public void ReadingAndWritingIgnoreTheMachinesCulture()
    {
        ...
        try
        {
            ...
            DateTime moment = CompassMoments.MakeMoment(2026, 12, 25, 9, 5, 0);
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(
                    CompassMoments.Format(moment, "yyyy/MM/dd HH:mm"),
                    Is.EqualTo("2026/12/25 09:05"));

                Assert.That(
                    CompassMoments.ParseMoment("2026-12-25 09:05").Value,
                    Is.EqualTo(moment));

                Assert.That(
                    CompassMoments.ParseDateExactly("25/12/2026", "dd/MM/yyyy").Value,
                    Is.EqualTo(CompassMoments.MakeDate(2026, 12, 25)));
            });
        }
        finally
        {
    ...
    public void TextThatDoesNotReadIsAbsent()
    {
        Assert.Multiple(() =>
        {
            Assert.That(CompassMoments.ParseMoment("the fifteenth").HasValue, Is.False);
            Assert.That(CompassMoments.ParseDate("nonsense").HasValue, Is.False);
            Assert.That(CompassMoments.ParseTime("half past").HasValue, Is.False);
            Assert.That(CompassMoments.ParseSpan("a while").HasValue, Is.False);

            // And the untyped forms the interpreter reads, which say the same with null.
            Assert.That(CompassMoments.ParseMomentUntyped("the fifteenth"), Is.Null);
            Assert.That(CompassMoments.ParseDateUntyped("2026-08-15"), Is.Not.Null);
        });
    }
    ...
    {
        TimeOnly late = CompassMoments.TimeAddHours(CompassMoments.MakeTime(17, 30), 8m);

        Assert.That(CompassMoments.TimeHour(late), Is.EqualTo(1L));
        Assert.That(CompassMoments.TimeMinute(late), Is.EqualTo(30L));
    }
}
