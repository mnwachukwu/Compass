...
/// </summary>
[TestFixture]
public sealed class CompassMathTests
{
    /// <summary>
    ...
    public void RoundingAFloatThatNamesNoNumberStops()
    {
        Assert.Multiple(() =>
        {
            ...
            {
                Assert.Throws<ArgumentException>(() => CompassMath.Floor(value));
                Assert.Throws<ArgumentException>(() => CompassMath.Ceiling(value));
                Assert.Throws<ArgumentException>(() => CompassMath.Round(value));
            }

            // The refusal names which of the two it met, since the fixes differ.
            Assert.That(
                Assert.Throws<ArgumentException>(
                    () => CompassMath.Round(double.PositiveInfinity))!.Message,
                Does.Contain("infinity"));

            Assert.That(
                Assert.Throws<ArgumentException>(() => CompassMath.Round(double.NaN))!.Message,
    ...
    public void AnOrdinaryFloatStillRounds()
    {
        Assert.Multiple(() =>
        {
            Assert.That(CompassMath.Floor(2.9), Is.EqualTo(2));
            Assert.That(CompassMath.Ceiling(2.1), Is.EqualTo(3));
            Assert.That(CompassMath.Round(2.5), Is.EqualTo(3));
            Assert.That(CompassMath.Round(-2.5), Is.EqualTo(-3));

            // Kept to places, a float stays a float — so an infinity is a float it can hold and
            // there is nothing here to refuse.
            Assert.That(CompassMath.Round(double.PositiveInfinity, 2),
    ...
    public void ARealAndAFractionRoundAsTheyDid()
    {
        Assert.Multiple(() =>
        {
            Assert.That(CompassMath.Floor(2.9m), Is.EqualTo(2));
            Assert.That(CompassMath.Ceiling(2.1m), Is.EqualTo(3));
            Assert.That(CompassMath.Round(2.5m), Is.EqualTo(3));

            Assert.That(CompassMath.Round(new Fraction(5, 2)), Is.EqualTo(3));
            Assert.That(CompassMath.Floor(new Fraction(5, 2)), Is.EqualTo(2));
            Assert.That(CompassMath.Ceiling(new Fraction(5, 2)), Is.EqualTo(3));
        });
    }
}
