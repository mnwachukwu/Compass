...
/// <para>Set <c>COMPASS_UPDATE_GOLDEN=1</c> to rewrite the recorded files after an intended
/// change.</para>
/// </summary>
[TestFixture]
public sealed class NegativeSampleTests : LexerTestBase
{
    ...
    private static string GoldenDirectory =>
        Path.Combine(RepositoryRoot, "tests", "Compass.Tests", "TestData", "Negatives");

    private static string NegativeDirectory(string kind) =>
        Path.Combine(RepositoryRoot, "samples", "negatives", kind);

    private static IEnumerable<string> NamesIn(string kind, string pattern = "*.cm") =>
    ...
    public static IEnumerable<string> ProjectFailureNames => NamesIn("project", "*.cmp");
    ...
    private static void AssertMatchesGolden(string actual, string goldenName, string sampleName)
    {
        ...
        Assert.That(
            actual,
            Is.EqualTo(expected),
            $"how {sampleName} fails changed; re-run with COMPASS_UPDATE_GOLDEN=1 if intended");
    }
}
