...
/// has typed <c>word.</c> has written something that is not Compass: there is no member yet, so
...
public sealed class CompletionTests
{
    ...
    private sealed class Workspace : IDisposable
    {
        ...
        {
            Folder = Path.Combine(Path.GetTempPath(), $"compass-completion-{Guid.NewGuid():N}");
            Directory.CreateDirectory(Folder);

            File.WriteAllText(Path.Combine(Folder, "Program.cm"), body);

            if (alongside is not null)
            {
                File.WriteAllText(Path.Combine(Folder, "Beside.cm"), alongside);
            }
        }
        ...
        public string At => Path.Combine(Folder, "Program.cm");
    ...
    /// force do not depend on the line being finished — or on it being Compass at all.</para>
    /// </summary>
    [Test]
