...
/// <para>Compass writes its types down, and <c>let</c> is the one place it does not — so unlike a
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                Hints.Wanted(JsonNode.Parse("""
                    {"compass": {"inlayHints": {"parameterNames": true}}}
                    """) as JsonObject),
