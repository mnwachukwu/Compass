...
public sealed class OccurrencesTests
{
    private static (CompilationUnit Unit, SemanticModel Model, SourceText Source) Compile(
        string text)
    {
        SourceText source = new(text, "Program.cm");
    ...
    private sealed class Workspace : IDisposable
    {
        ...
        {
            Folder = Path.Combine(Path.GetTempPath(), $"compass-uses-{Guid.NewGuid():N}");
            Directory.CreateDirectory(Folder);

            File.WriteAllText(Path.Combine(Folder, "Program.cm"), program);
            File.WriteAllText(Path.Combine(Folder, "Greeting.cm"), beside);
        }
    ...
    public void EveryUseIsFoundAcrossEveryFile()
    {
        ...
        Assert.That(
            Uses(workspace, "Greeting.cm", 2, 40),
            Is.EquivalentTo(new[]
            {
                ("Greeting.cm", 2),
                ("Program.cm", 3),
    ...
    public void TheDeclarationIsLeftOutWhenItWasNotAskedFor()
    {
        ...
        Assert.That(
            Uses(workspace, "Greeting.cm", 2, 40, includingDeclaration: false),
            Is.EquivalentTo(new[] { ("Program.cm", 3), ("Program.cm", 4) }));
    }
    ...
    public void ANameTheLanguageOwnsIsAnsweredFor()
    {
        ...
        Assert.That(
            Uses(workspace, "Program.cm", 3, 17),
            Is.EquivalentTo(new[] { ("Program.cm", 3), ("Program.cm", 4) }));
    }
