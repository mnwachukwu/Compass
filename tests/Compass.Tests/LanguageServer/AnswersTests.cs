...
public sealed class AnswersTests
{
    ...
    private sealed class Workspace : IDisposable
    {
        public Workspace()
        {
            Folder = Path.Combine(Path.GetTempPath(), $"compass-answers-{Guid.NewGuid():N}");
            Directory.CreateDirectory(Folder);

            File.WriteAllText(Path.Combine(Folder, "Program.cm"), Program);
            File.WriteAllText(Path.Combine(Folder, "Greeting.cm"), Greeting);
        }
    ...
    {
        SourceText source = new(Program, "Program.cm");
        DiagnosticBag diagnostics = new();
    ...
    public void ANameDeclaredElsewhereIsFoundThere()
    {
        ...
        // Line 3, column 36: inside 'Length', which Greeting.cm declares.
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That((string?)found[0]!["uri"], Does.Contain("Greeting.cm"));
    ...
    public void ALocalIsFoundWhereItWasDeclared()
    {
        ...
        {
            Assert.That(found, Has.Count.EqualTo(1));
            Assert.That((string?)found[0]!["uri"], Does.Contain("Program.cm"));
            Assert.That(
    ...
    public void FollowingANameSelectsTheNameAlone()
    {
        ...
        Assert.Multiple(() =>
        {
            // 'Length' on line 2 of Greeting.cm, at columns 36 through 41 as a reader counts.
    ...
    public void HoveringAParameterSaysWhatWasWrittenAboutThatParameter()
    {
        ...
        CompilationUnit greeting = units.Single(
            u => Path.GetFileName(u.Source.FileName) == "Greeting.cm");
    ...
    public void ACallSaysWhatItTakes()
    {
        ...
        File.WriteAllText(workspace.At("Program.cm"), Adding);
    ...
    public void ACallSaysWhatItIsForAndWhatEachParameterIsFor()
    {
        ...
        using Workspace workspace = new();
        workspace.Write("Program.cm", Documented);
    ...
    public void AHoverSaysTheShapeFirstAndWhereItCameFromLast()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(said, Does.StartWith("```compass"), "the shape, first");
            Assert.That(
                said.TrimEnd(),
    ...
    {
        using Workspace workspace = new();
        workspace.Write("Program.cm", Documenting);
