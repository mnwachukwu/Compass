...
public sealed class WorkspaceFoldersTests
{
    ...
    /// the compiler is matched through <see cref="Compass.Cli.SourceDiscovery.PathComparer"/>,
    ...
    private static void AssertFolders(WorkspaceFolders open, params string[] expected) =>
        ...
        Assert.That(
            ...
            Is.EqualTo(expected).Using(
                (IEqualityComparer<string>)Compass.Cli.SourceDiscovery.PathComparer));
    ...
    private static (string Uri, string Path) Somewhere =>
        OnWindows
            ? ("file:///d%3A/Repos/Compass", @"D:\Repos\Compass")
            : ("file:///home/matt/Compass", "/home/matt/Compass");
    ...
    /// <para><c>D:\Repos\Compass-Editors</c> begins the same way as <c>D:\Repos\Compass</c> and is
    /// not inside it. A prefix test answers yes, and would put a neighbouring checkout inside the one
    /// being searched.</para>
    /// </summary>
    [Test]
    public void AFolderBeginningTheSameWayIsNotInside()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(open.Holds(Path.Combine(path, "samples", "hello.cm")), Is.True);
    ...
    public void TheServerAsksToBeToldAboutFolders()
    {
        ...
        using (Compass.Cli.LanguageServer.LanguageServer server = new(
