...
public sealed class RenameTests
{
    ...
    private sealed class Workspace : IDisposable
    {
        ...
        {
            Folder = Path.Combine(Path.GetTempPath(), $"compass-rename-{Guid.NewGuid():N}");
            ...
            {
                File.WriteAllText(Path.Combine(Folder, "Beside.cm"), beside);
            }
        }
        ...
        Compile(Workspace workspace)
    {
        ...
        SourceDiscovery.Compilation gathered =
            SourceDiscovery.Gather(workspace.At("Program.cm"), diagnostics)!;
        ...
        return (
            gathered.Units,
            model,
            gathered.Units.Single(u => Path.GetFileName(u.Source.FileName) == "Program.cm"));
    }
    ...
    public void ALocalIsRenamedEverywhereItIsWritten()
    {
        ...
        JsonArray edits = EditsIn(change, workspace.At("Program.cm"));
    ...
    public void AModelAndItsConstructorAreOneName(int line, int column)
    {
        ...
        CompilationUnit beside =
            units.Single(u => Path.GetFileName(u.Source.FileName) == "Beside.cm");

        JsonObject? change = Rename.Edits(
            units, model, beside, OffsetOf(beside.Source, line, column), "Volume");

        JsonArray edits = EditsIn(change, workspace.At("Beside.cm"));
    ...
    public void AFunctionIsRenamedAcrossEveryFile()
    {
        ...
        {
            Assert.That(
                Applied(Program, EditsIn(change, workspace.At("Program.cm"))),
                Does.Contain("Greeting.Size(\"hello\")"));

            Assert.That(
                Applied(Greeting, EditsIn(change, workspace.At("Greeting.cm"))),
