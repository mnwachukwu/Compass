...
/// <see cref="Compass.Services.Rename"/> directly, because what changed is which
...
public sealed class CrossProjectTests
{
    ...
    private sealed class Workspace : IDisposable
    {
        public Workspace()
        {
            ...
            Write(
                Path.Combine("library", "library.cmp"),
                """
                project Library
                    reference ../books/books.cmp
                    source Program.cm
                end project
                """);

            Write(
                Path.Combine("library", "Program.cm"),
                """
    ...
    private static JsonNode? Answer(int id, params JsonObject[] messages)
    {
        ...
        using (Compass.Cli.LanguageServer.LanguageServer server =
    ...
    public void RenamingFromTheDeclarationReachesTheProjectBuiltOnIt()
    {
        ...
        JsonNode? answered = Answer(
            ...
            Asking(2, "textDocument/rename", workspace.UriOf("books", "Book.cm"), DeclaredAt,
        ...
        Assert.Multiple(() =>
        {
            Assert.That(changes.ContainsKey(workspace.UriOf("books", "Book.cm")), Is.True,
                        "the file declaring it");

            Assert.That(changes.ContainsKey(workspace.UriOf("library", "Program.cm")), Is.True,
    ...
    public void EachEditIsOfferedOnce()
    {
        ...
        JsonArray declaring =
            (JsonArray)((JsonObject)answered!["changes"]!)[workspace.UriOf("books", "Book.cm")]!;

        // The model's name and the constructor's, which is every place Book.cm writes it.
    ...
    /// <see cref="Compass.Cli.SourceDiscovery"/> was already gathering.
    /// </summary>
    [Test]
    public void RenamingFromTheUseStillReachesTheDeclaration()
    {
        ...
        JsonNode? answered = Answer(
            2,
            Initialize(workspace.Root),
            Asking(2, "textDocument/rename", workspace.UriOf("library", "Program.cm"), UsedAt,
                   new JsonObject { ["newName"] = "Volume" }));
    ...
    public void FindingUsesFromTheDeclarationReachesTheProjectBuiltOnIt()
    {
        ...
        JsonNode? answered = Answer(
            2,
            Initialize(workspace.Root),
            Asking(2, "textDocument/references", workspace.UriOf("books", "Book.cm"), DeclaredAt,
                   new JsonObject
                   {
        ...
        Assert.That(files, Does.Contain(workspace.UriOf("library", "Program.cm")));
        Assert.That(files, Does.Contain(workspace.UriOf("books", "Book.cm")));
    }
    ...
    public void WithNoWorkspaceTheAnswerStopsAtTheOneCompilation()
    {
        ...
        Assert.That(changes.ContainsKey(workspace.UriOf("library", "Program.cm")), Is.False,
