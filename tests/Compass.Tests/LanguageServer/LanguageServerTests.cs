...
public sealed class LanguageServerTests
{
    ...
    private sealed class Workspace : IDisposable
    {
        public Workspace()
        {
            Folder = Path.Combine(Path.GetTempPath(), $"compass-lsp-{Guid.NewGuid():N}");
    ...
    public void ItSaysWhatItCanDo()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(answered, Does.Contain("\"name\":\"compass\""));
    ...
    public void ItSaysWhichBlocksFold()
    {
        ...
        string uri = workspace.UriOf("Program.cm");
        ...
        string answered = Answering(
            Framed(
                ...
                Open(uri, File.ReadAllText(workspace.At("Program.cm"))),
                new JsonObject
                {
                    ["jsonrpc"] = "2.0",
                    ["id"] = 11,
    ...
    public void TheEditorsTextIsWhatIsCompiled()
    {
        ...
        string answered = Answering(
            Framed(Open(
                ...
                """)),
            text => text.Contains("CM0026", StringComparison.Ordinal));

        JsonArray found = Published(answered)[workspace.UriOf("Program.cm")];

        Assert.Multiple(() =>
        {
            Assert.That(found, Has.Count.EqualTo(1));
            Assert.That((string?)found[0]!["code"], Is.EqualTo("CM0026"));
            Assert.That((int?)found[0]!["severity"], Is.EqualTo(1));
            Assert.That((string?)found[0]!["source"], Is.EqualTo("compass"));
    ...
    public void EveryFileInTheProgramIsPublishedIncludingTheCleanOnes()
    {
        ...
        string answered = Answering(
            Framed(Open(workspace.UriOf("Program.cm"), File.ReadAllText(workspace.At("Program.cm")))),
            text => text.Contains("Greeting.cm", StringComparison.Ordinal));

        Dictionary<string, JsonArray> published = Published(answered);

        Assert.Multiple(() =>
        {
            Assert.That(published[workspace.UriOf("Program.cm")], Is.Empty);
            Assert.That(
                published[workspace.UriOf("Greeting.cm")],
                Is.Empty,
            ...
            Framed(
                ...
                new JsonObject
                {
                    ...
                    ["params"] = new JsonObject
                    {
                        ["textDocument"] = new JsonObject
                        {
                            ["uri"] = workspace.UriOf("Program.cm"),
                        },
    ...
    public void CompletionAnswersAboutTheBufferAndNotTheSavedFile()
    {
        ...
        JsonObject Asking(int id, int line, int character) => new()
        {
            ...
            ["params"] = new JsonObject
            {
                ["textDocument"] = new JsonObject { ["uri"] = workspace.UriOf("Program.cm") },
        ...
        string answered = Answering(
            Framed(
                Open(workspace.UriOf("Program.cm"), typing),
    ...
    /// looking.</para>
    /// <para>A program is a compilation. Editing <c>Program.cm</c> can break <c>Greeting.cm</c>,
    ...
    public void AMistakeIsReportedAgainstTheFileItIsIn()
    {
        ...
        string answered = Answering(
            Framed(Open(workspace.UriOf("Program.cm"), File.ReadAllText(workspace.At("Program.cm")))),
            text => text.Contains("CM03", StringComparison.Ordinal));
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                published[workspace.UriOf("Program.cm")],
                Is.Empty,
