using Compass.Cli;
using Compass.Cli.LanguageServer;
using Compass.Compiler.Diagnostics;
using Compass.Services;

namespace Compass.Tests.LanguageServer;
...
/// <c>Program.cm</c> re-analyzes the file beside it that only the disk knows — and an open file
...
public sealed class DocumentStoreTests
{
    ...
    private sealed class Workspace : IDisposable
    {
        public Workspace()
        {
            Folder = Path.Combine(Path.GetTempPath(), $"compass-store-{Guid.NewGuid():N}");
    ...
    public void AFileTheEditorHasNotOpenedIsRead()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(store.Find("nothing.cm"), Is.Null);
    ...
    public void AnOpenFileIsHeldAndFoundAgain()
    {
        ...
        store.Set("a.cm", "shared model Program\nend model\n", version: 1);

        Assert.Multiple(() =>
        {
            Assert.That(store.Find("a.cm")!.Text, Does.Contain("shared model Program"));
            Assert.That(store.Find("a.cm")!.Version, Is.EqualTo(1));
    ...
    public void AnEditReplacesWhatWasThere()
    {
        ...
        store.Set("a.cm", "first", version: 1);
        store.Set("a.cm", "second", version: 4);

        Assert.Multiple(() =>
        {
            Assert.That(store.Find("a.cm")!.Text, Is.EqualTo("second"));
            Assert.That(store.Find("a.cm")!.Version, Is.EqualTo(4), "and versions need not run by one");
    ...
    public void OneFileNamedTwoWaysIsOneFile()
    {
        ...
        store.Set(workspace.At("Program.cm"), "held", version: 1);
        store.Set(Path.Combine(workspace.Folder, ".", "Program.cm"), "held again", version: 2);

        Assert.Multiple(() =>
        {
            Assert.That(store.Count, Is.EqualTo(1));
            Assert.That(store.Find(workspace.At("Program.cm"))!.Text, Is.EqualTo("held again"));
        });
    }
    ...
    public void AForwardSlashNamesTheSameFileAsTheSeparatorDoes()
    {
        ...
        store.Set(workspace.At("Program.cm"), "held", version: 1);
        store.Set($"{workspace.Folder}/Program.cm", "held again", version: 2);
    ...
    public void CaseIsTreatedTheWayThisPlatformTreatsIt()
    {
        ...
        store.Set(workspace.At("Program.cm"), "held", version: 1);
        store.Set(workspace.At("PROGRAM.CM"), "held again", version: 2);
    ...
    public void ClosingHandsAFileBackToTheDisk()
    {
        ...
        store.Set("a.cm", "held", version: 1);

        Assert.Multiple(() =>
        {
            Assert.That(store.Close("a.cm"), Is.True);
            Assert.That(store.Find("a.cm"), Is.Null);
            Assert.That(store.Close("a.cm"), Is.False, "and closing it twice says so");
        });
    }
    ...
    /// </para>
    /// <para>The whole point, in one assertion. <c>Greeting.cm</c> is edited but never saved, and
    /// what compiles is the edit; <c>Program.cm</c> is never opened, and it still arrives.</para>
    /// </summary>
    [Test]
    public void ACompilationReadsTheEditorFirstAndTheDiskAfter()
    {
        ...
        SourceDiscovery.Compilation gathered =
            SourceDiscovery.Gather(workspace.At("Program.cm"), diagnostics, store.Reader)!;

        string greeting = gathered.Units
            .Single(unit => unit.Source.FileName.EndsWith("Greeting.cm", StringComparison.Ordinal))
            .Source.Text;
    ...
    public void WithNothingOpenTheDiskIsRead()
    {
        ...
        SourceDiscovery.Compilation gathered =
            SourceDiscovery.Gather(workspace.At("Program.cm"), diagnostics, new DocumentStore().Reader)!;

        Assert.That(
            gathered.Units.Single(u => u.Source.FileName.EndsWith("Greeting.cm", StringComparison.Ordinal))
                          .Source.Text,
