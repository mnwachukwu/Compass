...
/// question lives: <see cref="Compass.Compiler.Emit.CilEmitter"/> writes wherever it is told, and
...
public sealed class BuildCommandTests
{
    ...
    private sealed class Workspace : IDisposable
    {
        public Workspace()
        {
            Folder = Path.Combine(Path.GetTempPath(), $"compass-build-{Guid.NewGuid():N}");
            Directory.CreateDirectory(Folder);

            Program = Path.Combine(Folder, "Hello.cm");
    ...
    private static (int Code, string Said) Build(params string[] args)
    {
        ...
        try
        {
            ...
            return (Compass.Cli.Program.Run(args), said.ToString());
        }
        finally
        {
    ...
    public void EverythingNeededToRunGoesTogether()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(written, Does.Contain("Compass.Runtime.dll"));
        });
    }
    ...
    public void AProjectSaysWhereItsBuildGoes()
    {
        ...
        string project = workspace.Write("Hello.cmp", """
            project Hello
                source Hello.cm
    ...
    public void AProjectsOutputIsReadFromTheProjectsFolder()
    {
        ...
        File.Move(workspace.Program, Path.Combine(workspace.Folder, "inner", "Hello.cm"));

        string project = workspace.Write(Path.Combine("inner", "Hello.cmp"), """
            project Hello
                source Hello.cm
                output ../up-one
            end project
    ...
    public void AnOutputWithNoFolderIsRefused()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(said, Does.Contain("CM0628"));
        });
    }
    ...
    public void TwoOutputsAreRefused()
    {
        ...
        {
            Assert.That(code, Is.EqualTo(1));
            Assert.That(said, Does.Contain("CM0629"));
        });
    }
    ...
    public void SomethingWithNoProgramBuildsAsALibrary()
    {
        ...
        string source = workspace.Write("Greeting.cm", NoProgram);
    ...
    public void ItSaysWhenWhatItBuiltIsALibrary()
    {
        ...
        (int code, string said) = Build("build", workspace.Write("Greeting.cm", NoProgram));
    ...
    public void RunningSomethingWithNoProgramIsStillRefused()
    {
        ...
        (int code, string said) = Build("run", workspace.Write("Greeting.cm", NoProgram));

        Assert.Multiple(() =>
        {
            Assert.That(code, Is.EqualTo(1));
            Assert.That(said, Does.Contain("CM0212"));
        });
    }
}
