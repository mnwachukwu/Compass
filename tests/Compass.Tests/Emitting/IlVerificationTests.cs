...
namespace Compass.Tests.Emitting;
...
public sealed class IlVerificationTests : LexerTestBase
{
    private static IEnumerable<string> Programs =>
        Compass.Tests.Interpreting.SampleProgramTests.RunnableSampleNames;
    ...
    [
        Path.Combine("reference", "tour.cm"),
        Path.Combine("library", "books", "books.cmp"),
    ];
    ...
    /// for as long as it existed, and how a nested type came to crash <c>cm build</c> while
    ...
    {
        HashSet<string> reached = new(Compass.Cli.SourceDiscovery.PathComparer);
                                             ...
                                             .Select(e => Path.Combine("samples", e))))
        {
            ...
            if (Compass.Cli.SourceDiscovery.Gather(path, diagnostics) is not { } gathered)
            {
                continue;
            }
    ...
    /// <para>Set <c>COMPASS_KEEP_ASSEMBLIES=1</c> and every compilation below is written under
    ...
    private static bool Keeping =>
        Environment.GetEnvironmentVariable("COMPASS_KEEP_ASSEMBLIES") == "1";

    /// <summary>Where kept assemblies go, which is where <c>cm build</c> would put one.</summary>
    ...
    /// <summary>A sample's name as a folder: 'library/books/books.cmp' becomes 'library-books'.
    /// </summary>
        ...
        string name, Func<(IReadOnlyList<CompilationUnit> Units, SemanticModel Model)> frontEnd)
    {
        ...
        string folder = keeping
            ...
            : Path.Combine(Path.GetTempPath(), $"compass-ilverify-{Guid.NewGuid():N}");
    ...
    /// <c>cm build</c> of the same path would produce, folder rules and project file and all.
    /// </summary>
        ...
        string entry, bool requireEntryPoint)
    {
        ...
        Compass.Cli.SourceDiscovery.Compilation compilation =
            Compass.Cli.SourceDiscovery.Gather(path, diagnostics)!;
    ...
    /// holds <c>Compass.Runtime</c>, and the folder being verified holds the assembly
    /// itself.</para>
    /// </summary>
