...
public sealed class CorpusAgreementTests : LexerTestBase
{
    ...
    private static IEnumerable<string> MultiFilePrograms =>
        Compass.Tests.Interpreting.MultiFileSampleTests.EntryPoints;
    ...
    /// this fixture, so what the two engines are handed is what a <c>cm build</c> of the same
        ...
        string entry)
    {
        ...
        Compass.Cli.SourceDiscovery.Compilation gathered =
        ...
        string name, (IReadOnlyList<CompilationUnit> Units, SemanticModel Model) compiled)
    {
        ...
        string folder = Path.Combine(Path.GetTempPath(), $"compass-corpus-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            Compass.Interpreter.Interpreter.Run(units, model, interpreted, TextReader.Null);
