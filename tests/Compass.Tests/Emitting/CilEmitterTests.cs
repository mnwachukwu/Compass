...
public sealed class CilEmitterTests
{
    ...
    private static string Interpreted(string source)
    {
        ...
        Compass.Interpreter.Interpreter.Run(compiled.Emitting, compiled.Model, output);
    ...
    private static string Emitted(string source)
    {
        ...
        string folder = Path.Combine(Path.GetTempPath(), $"compass-emit-{Guid.NewGuid():N}");
    ...
    /// since dividing by a written zero is caught while checking (<c>CM0324</c>) and the program
        ...
        AgreesWholly("""
            ...
            shared model Program
                ...
                function Main()
                    ...
                    # and asking is CM0347 rather than an answer.
    ...
    /// it proves beyond the values is the boundary: Compass counts in 64 bits and the list
    ...
    /// <summary>
    /// The one rule that separates a Compass string from a .NET one: an empty argument matches
    ...
    /// the one written to explain <c>CM0324</c>. The first is the worst by a distance: it is not a
