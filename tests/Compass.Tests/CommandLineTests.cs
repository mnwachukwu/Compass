...
/// <para>Both names, because <c>cm</c> is a second executable rather than a copy of the first:
...
public sealed class CommandLineTests : LexerTestBase
{
    ...
    public void CreateFolder()
    {
        _folder = Path.Combine(
            Path.GetTempPath(), "compass-cli", TestContext.CurrentContext.Test.ID);
    ...
    public static IEnumerable<TestCaseData> Commands
    {
        get
        {
            string tail = Path.GetRelativePath(
                Path.Combine(RepositoryRootForTests, "tests", "Compass.Tests"),
                AppContext.BaseDirectory);

            foreach ((string project, string name) in
                     new[] { ("Compass.Cli", "compass"), ("Compass.Cli.Alias", "cm") })
            {
    ...
    {
        Result result = Run(command, "run", Write("Program.cm", Greeting));
    ...
    {
        Result result = Run(command, "check", Write("Program.cm", Greeting));
    ...
    public void CheckingABadProgramFails(string command)
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(result.Output + result.Error, Does.Contain("CM0300"));
        });
    }
    ...
    {
        Result result = Run(command, "run", Path.Combine(_folder, "nowhere.cm"));
    ...
    public void TheExitCodeSaysWhichKindOfWrong()
    {
        ...
        string good = Write("Program.cm", Greeting);
        ...
        string bad = Write(Path.Combine("apart", "Program.cm"), """
            shared model Program
                function Main()
                    integer n = "not a number";
                end function
    ...
    /// <para>Read as a path, <c>build --help</c> answers "file not found: --help.cm" — a file
    ...
    public void EveryCommandInTheHelpAnswers()
    {
        ...
        [
            .. help.Output.Split('\n')
                ...
                .Where(words => words.Length > 1 && words[0] is "compass" or "cm")
    ...
    public void NewWritesAnEmptyProgramThatRuns(string command)
    {
        ...
        Result ran = Run(command, "run", Path.Combine(_folder, "hello.cm"));
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                File.ReadAllText(Path.Combine(_folder, "hello.cm")),
    ...
    public void SampleWritesAProgramThatDoesSomething(string command)
    {
        ...
        Result ran = Run(command, "run", Path.Combine(_folder, "tour.cm"));
    ...
    public void NewRefusesToWriteOverWhatIsThere(string command)
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                File.ReadAllText(Path.Combine(_folder, "taken.cm")),
    ...
    public void NewWritesAProjectThatRuns(string command)
    {
        ...
        Result ran = Run(command, "run", Path.Combine(_folder, "store", "store.cmp"));
    ...
    public void TheAliasAnswersAsTheCommandDoes()
    {
        ...
        string program = Write("Program.cm", Greeting);
