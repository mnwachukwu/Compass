...
public sealed class LiteralTests : LexerTestBase
{
    ...
    [TestCase("0b", "CM0017")]
    [TestCase("0xG", "CM0018")]
    [TestCase("0b12", "CM0018")]
    [TestCase("0xFFZZ", "CM0018")]
    ...
    public void ASeparatorWithNothingToGroup_IsReported(string source) => Assert.That(
        ScanRaw(source).Diagnostics.Select(d => d.Id), Is.EqualTo(new[] { "CM0020" }));
    ...
    public void AnExponentWithNoDigits_IsReported(string source) => Assert.That(
        ScanRaw(source).Diagnostics.Select(d => d.Id), Is.EqualTo(new[] { "CM0019" }));
    ...
    public void ANameWrittenAgainstANumber_IsOneMistake(string source)
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(diagnostics.Select(d => d.Id), Is.EqualTo(new[] { "CM0021" }));
