...
/// <para>The recorded format is the one <c>compass tokens</c> prints, so a golden file is
/// simultaneously a regression test and a worked example of a shipped command.</para>
...
public sealed class GoldenTokenStreamTests : LexerTestBase
{
    ...
    private static string GoldenDirectory =>
        Path.Combine(RepositoryRoot, "tests", "Compass.Tests", "TestData", "Lexing", "Golden");
    ...
    public void Sample_MatchesItsRecordedTokenStream(string name)
    {
        ...
        Assert.That(actual, Is.EqualTo(expected),
                    $"token stream for {name} changed; re-run with COMPASS_UPDATE_GOLDEN=1 if intended");
    }
}
