...
namespace Compass.Tests.Lexing;
...
public sealed class RecoveryTests : LexerTestBase
{
    ...
    public void UnrecognizedCharacter_IsReportedAndSkipped()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0001" }));
    ...
    public void SeveralUnrecognizedCharacters_AreAllReportedInOnePass()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0001", "CM0001", "CM0001", "CM0001" }));
    ...
    public void UnterminatedString_ReportsAtTheOpeningQuoteAndStopsAtTheNewline()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0002" }));
    ...
    public void UnterminatedCharacter_IsReportedAndRecovered()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0003" }));
    ...
    public void CharacterLiteral_MustHoldExactlyOneCharacter(string source)
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0004" }));
    ...
    public void UnterminatedBlockComment_IsReportedAtItsOpener()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0005" }));
    ...
    [TestCase("||", "or")]
    [TestCase("!", "not")]
    public void CSharpBooleanOperator_SuggestsTheCompassSpelling(string source, string suggestion)
    {
        (_, DiagnosticBag diagnostics) = ScanRaw($"a {source} b");

        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0006" }));
    ...
    /// multiplications, and pointed at the one Compass uses.
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// The two arrows other languages put before a lambda's body. Compass writes the word
    ...
    public void Increment_IsStillReported_BecauseThereIsNoUnaryPlus()
    {
        ...
        // Compass has no unary plus for the second "+" to be part of.
    ...
    public void UnrecognizedEscape_IsReported(string source)
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Does.Contain("CM0007"));
    ...
    public void MalformedUnicodeEscape_IsReported(string source)
    {
        ...
        Assert.That(IdsOf(diagnostics), Does.Contain("CM0008"));
    }
    ...
    public void SeveralIndependentErrors_AreAllReportedInOnePass()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics),
                        Is.EqualTo(new[] { "CM0001", "CM0004", "CM0002", "CM0006" }));
    ...
    public void DiagnosticBag_StopsCollectingAtItsCapAndSaysSo()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(diagnostics.Count(d => d.Id == "CM0113"), Is.EqualTo(1));
            Assert.That(diagnostics.Last().Id, Is.EqualTo("CM0113"));
        });
    }
