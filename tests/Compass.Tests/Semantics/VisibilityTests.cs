...
public sealed class VisibilityTests
{
    ...
    private static string[] CheckAcrossProjects(string first, string second)
    {
        ...
        CompilationUnit one = Parser.Parse(new SourceText(first, "Core.cm"), diagnostics);
        CompilationUnit two = Parser.Parse(new SourceText(second, "App.cm"), diagnostics);
        ...
        params string[] sources)
    {
        ...
        Compass.Interpreter.Interpreter.Run(Lowering.Lower(units, model), model, output);
    ...
    public void AnInternalMemberCarriesItsValueAcrossFilesInOneProject() => Assert.That(
        RunAcrossProjects(
            new Dictionary<string, string> { ["Box.cm"] = "Core", ["Program.cm"] = "Core" },
            """
            public model Box
    ...
    public void WithNoProjectFileInternalReachesEveryFile() => Assert.That(
        RunAcrossProjects(
            new Dictionary<string, string> { ["Helper.cm"] = "", ["Program.cm"] = "" },
            """
