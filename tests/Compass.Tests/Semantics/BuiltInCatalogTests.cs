...
public sealed class BuiltInCatalogTests
{
    ...
    public void NoBuiltInNameCanBeRedeclared(string name)
    {
        ...
        Assert.That(diagnostics.Select(d => d.Id), Does.Contain("CM0203"));
    }
    ...
    [TestCase("\"Compass\".ToUpper()", "COMPASS")]
    [TestCase("\"Compass\".ToLower()", "compass")]
    ...
    {
        Assert.That(
            Compass.Runtime.BuiltInExceptions.Resolve(name),
            Is.Not.Null,
        ...
        if (!Compass.Runtime.BuiltInExceptions.MayBeCaught(name))
        {
            return;
        }
    ...
    public static IEnumerable<string> ExceptionNames => Compass.Runtime.BuiltInExceptions.Names;

    public static IEnumerable<string> UncatchableExceptionNames =>
        Compass.Runtime.BuiltInExceptions.Uncatchable;
    ...
    /// Compass type it is running.</para>
    /// </summary>
    [Test]
    ...
    public void TheSpecificationListsEveryBuiltInException() => Assert.That(
        Compass.Runtime.BuiltInExceptions.Names.Where(
    ...
    public void TheSpecificationSaysHowManyThereAre() => Assert.That(
        ...
        Does.Contain(
            $"{Written(Compass.Runtime.BuiltInExceptions.Names.Count)} exception types, and "
            + "every one descends from `Exception`"));

    /// <summary>
    /// <para>The specification names every model that holds no values, and says how many.</para>
    /// <para>These are the ones <c>CM0233</c> refuses where a value's type belongs, so a reader
    ...
    public void TheSpecificationOpensSectionElevenWithEveryNameInStandard()
    {
        ...
        IEnumerable<string> named = BuiltIns.AllTypeNames.Where(
            name => name == "Exception" || !Compass.Runtime.BuiltInExceptions.Names.Contains(name));
