...
/// <c>samples/negatives/compile/numbers.cm</c> instead, where a reader can see it in a
/// program.</para>
/// </summary>
[TestFixture]
public sealed class NumberTooLargeTests
{
    ...
    public void ANumberTooLargeIsReported(string written) =>
        Assert.That(IdsIn(written), Is.EqualTo(new[] { "CM0026" }));
    ...
    /// <c>CM0324</c> also leaves alone — so refusing the literal would have the language
    ...
    public void AnIntegerLiteralIsNotWidenedToFitTheTypeWanted() =>
        Assert.That(IdsIn("        real r = 99999999999999999999;"), Is.EqualTo(new[] { "CM0026" }));
    ...
    public void TheMostNegativeIntegerIsReportedAgainstItsMinusSign()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(reported.Select(d => d.Id), Is.EqualTo(new[] { "CM0026" }));
    ...
    public void AFractionOverZeroIsReportedAsDivisionRatherThanSize() =>
        Assert.That(IdsIn("        fraction f = 1|0;"), Is.EqualTo(new[] { "CM0027" }));
    ...
    public void EverythingElseStillRefusesOne(string written) =>
        Assert.That(IdsIn(written), Is.EqualTo(new[] { "CM0324" }));
    ...
    public void AnEnumerationOrdinalIsChecked()
    {
        ...
        Assert.That(diagnostics.Sorted().Select(d => d.Id), Does.Contain("CM0026"));
    }

    /// <summary>
    /// <para>A constant built from one is told the real reason, and only that.</para>
    /// <para>It does not fold, so the constant check would otherwise add <c>CM0321</c> — "can
    ...
    public void AConstantBuiltFromOneIsNotAlsoToldItIsNotAConstant()
    {
        ...
        Assert.That(diagnostics.Sorted().Select(d => d.Id), Is.EqualTo(new[] { "CM0026" }));
    }
