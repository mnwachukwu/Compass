...
/// <c>samples/negatives/compile/throwaway.cm</c>, where a reader can see it in a program.</para>
/// </summary>
[TestFixture]
public sealed class ThrowawayTests
{
    ...
    public void ReadingAThrowawayIsRefused(string body) =>
        Assert.That(IdsIn(body), Does.Contain("CM0254"));
    ...
    public void AThrowawayWhereNoNameIsAskedForIsRefused(string body) =>
        Assert.That(IdsIn(body), Does.Contain("CM0256"));
    ...
    public void TheValueOfARefusedThrowawayIsStillChecked() =>
        Assert.That(IdsIn("        let _ = Program.Missing();"), Does.Contain("CM0306"));
    ...
    public void ALocalNothingReadsIsReported(string body) =>
        Assert.That(IdsIn(body), Does.Contain("CM0409"));
