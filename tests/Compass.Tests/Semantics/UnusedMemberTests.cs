...
/// <c>samples/negatives/compile/throwaway.cm</c>.</para>
/// </summary>
[TestFixture]
public sealed class UnusedMemberTests
{
    ...
    public void APrivateMemberNothingReachesIsReported(string member) =>
        Assert.That(IdsIn(Holding(member)), Does.Contain("CM0410"));
    ...
    public void ADeclarationDoesNotCountAsAUseOfItself() =>
        Assert.That(IdsIn(Holding("    integer Count = 3;")), Is.EqualTo(new[] { "CM0410" }));
    ...
    public void SharedOnAMemberOfASharedModelIsRemarkedOn(string member) =>
        Assert.That(IdsIn(Holding(member)), Does.Contain("CM0259"));
    ...
    public void SharedOnANestedTypeIsNotRemarkedOn(string nested) =>
        Assert.That(IdsIn(Holding(nested)), Does.Not.Contain("CM0259"));
    ...
    /// </para>
    /// <para>Each of these is written on its own in <c>samples/exceptions.cm</c> to raise what it
