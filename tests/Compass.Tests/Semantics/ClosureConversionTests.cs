...
/// produce ordinary Compass rather than a shape only an emitter would understand.</para>
/// </summary>
[TestFixture]
public sealed class ClosureConversionTests : LexerTestBase
{
    ...
    private static string RunOnce(CompilationUnit tree, SemanticModel model)
    {
        ...
        Compass.Interpreter.Interpreter.Run(tree, model, output, TextReader.Null);
    ...
    /// <c>samples/looping.cm</c> exists to say Compass does not have.</para>
    /// </summary>
    [Test]
    ...
    public void FramesFromDifferentFilesDoNotShareAName()
    {
        ...
        StringWriter output = new();
        Compass.Interpreter.Interpreter.Run(converted, model, output, TextReader.Null);
