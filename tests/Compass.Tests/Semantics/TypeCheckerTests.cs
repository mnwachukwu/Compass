...
public sealed class TypeCheckerTests
{
    ...
    public void ARealWidensToAFractionButNotTheOtherWayAround()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(toReal), Is.EqualTo(new[] { "CM0301" }));
    ...
    /// left to fail when it runs. <c>CM0324</c> draws the same line around dividing by zero.
    /// </summary>
    [Test]
    ...
    public void AnOptionalMustBeUnwrappedAndTheMessageSaysHow()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0329" }));
    ...
    public void AnOptionalStillDoesNotReachAPlainValue(string body) =>
        Assert.That(IdsOf(CheckBody(body)), Does.Contain("CM0329"));
    ...
    public void AValueTypeNeverConvertsToModel(string body) =>
        Assert.That(IdsOf(CheckBody(body)), Is.EqualTo(new[] { "CM0300" }));
    ...
    public void ConditionalBranchesMustAgreeExactly()
    {
        ...
        Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0305" }));
    }
    ...
    public void IndexingNeedsASetOrAStringAndAnIntegerIndex()
    {
        Assert.Multiple(() =>
        {
            ...
            Assert.That(IdsOf(CheckBody("        let bad = 5[0];")),
                        Is.EqualTo(new[] { "CM0311" }));

            Assert.That(IdsOf(CheckBody(
                """
                        integer[] numbers = {1};
                        let bad = numbers["x"];
                """)), Is.EqualTo(new[] { "CM0312" }));
        });
    }
    ...
    public void CheckingOneAClosureCanUndoIsSaidToBeNoHelp()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(reported), Is.EqualTo(new[] { "CM0345" }));
    ...
    public void ASwitchCannotExamineATypeWhereEqualityIsUnreliable(string type, string value)
    {
        ...
        Assert.That(IdsOf(diagnostics), Does.Contain("CM0315"));
    }
    ...
    public void ASwitchLeavingEnumerationMembersOutIsReported()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0337" }));
    ...
    public void ALabelThatDidNotLandSuppressesTheExhaustivenessWarning(string body) =>
        Assert.That(IdsOf(CheckSuit(body)), Does.Not.Contain("CM0337"));
    ...
    public void AConditionMessageNamesItsSubjectCorrectly(string body, string expected)
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Does.Contain("CM0302"));
            Assert.That(diagnostics.First(d => d.Id == "CM0302").Message,
    ...
    public void UsingTheResultOfAFunctionThatYieldsNothingIsRejected(string body) =>
        Assert.That(IdsOf(CheckBody(body)), Is.EqualTo(new[] { "CM0332" }));
    ...
    [TestCase("        integer x = \"text\";", "CM0300")]
    [TestCase("        let y = true * 2;", "CM0303")]
    ...
    public void ARangeLoopStillCountsWithIntegers(string body) =>
        Assert.That(IdsOf(CheckBody(body)), Is.EqualTo(new[] { "CM0317" }));
    ...
    public void ABuiltInMemberHasToBeCalled(string body) =>
        Assert.That(IdsOf(CheckBody(body)), Is.EqualTo(new[] { "CM0330" }));
    ...
    [TestCase("        integer[] xs = {1};\n        let n = xs.Count();", new[] { "CM0338" })]
    [TestCase("        let n = \"abc\".Count();", new[] { "CM0338" })]
    ...
    public void ABareParameterNameWithNothingToTakeATypeFromIsReported(string body) =>
        Assert.That(IdsOf(CheckBody(body)), Does.Contain("CM0336"));
    ...
    public void AWrittenTypeTheSurroundingCodeAlreadyGaveIsReported(string body, int count) =>
        Assert.That(
            ...
            Is.EqualTo(Enumerable.Repeat("CM0115", count)));
    ...
    public void AMixedListIsReportedOnlyForTheTypeThatWasWritten()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0115" }));
    ...
    [TestCase("        integer delegate(integer)? f = (integer n) yield n + 1;", new[] { "CM0115" })]
    ...
    /// <c>CM0336</c>, while everywhere else it is the other way round. There is no third
    ...
    public void NothingElseReachesFunction(string body) =>
        Assert.That(IdsOf(CheckBody(body)), Does.Contain("CM0300"));
    ...
    /// </summary>
    [TestCase("        Thing t = new Thing(\"text\");", "CM0300")]
    [TestCase("        Thing t = new Thing(1, 2);", "CM0308")]
    [TestCase("        Thing t = new Thing();", "CM0308")]
    ...
    [TestCase("        Bare b = new Bare(1);", new[] { "CM0308" })]
    ...
    [TestCase("        let e = new Exception(1, 2);", new[] { "CM0308" })]
    ...
    public void ATypeTheLanguageProvidesIsNotConstructedUnlessItSaysSo(string body) =>
        Assert.That(IdsOf(CheckBody(body)), Does.Contain("CM0328"));
    ...
    [TestCase("        Random r = new Random(1, 2);", new[] { "CM0308" })]
    ...
    // count — which is why this reports a type and not CM0308.
    [TestCase("        DateTime d = new DateTime(2026);", new[] { "CM0300" })]
    [TestCase("        DateTime d = new DateTime(2026, 7);", new[] { "CM0300", "CM0300" })]
    ...
    [TestCase("        let y = new DateTime(2026, 7, 29).Year();", new[] { "CM0338" })]
    [TestCase("        let n = new Random(1).Next();", new string[0])]
    [TestCase("        let n = new Random(1).Next;", new[] { "CM0330" })]
