...
public sealed class StatementParsingTests : ParserTestBase
{
    ...
    public void WritingATypeOnTheCounterIsRejectedAndOnlyOnce(string type)
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0111" }));
    ...
    {
        (_, Compass.Compiler.Diagnostics.DiagnosticBag diagnostics) = ParseRaw(
            $$"""
            shared model Program
                function Main()
            {{declaration}}
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0110" }));
