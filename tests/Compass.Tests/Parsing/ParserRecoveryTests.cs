...
namespace Compass.Tests.Parsing;
...
public sealed class ParserRecoveryTests : ParserTestBase
{
    ...
    public void MismatchedEndNamesBothTheCloserAndTheConstruct()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0104" }));
    ...
    public void UnterminatedConstructIsReportedAtItsOpener()
    {
        ...
        Assert.That(IdsOf(diagnostics), Does.Contain("CM0105"));
    }
    ...
    /// from a Compass program written before the word existed. Without the case, <c>for</c>
    ...
    public void ALoopWithoutItsOpenerIsOneDiagnosticNamingTheRewrite(string body, string rewrite)
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0120" }),
    ...
    public void StatementCannotBeginWithParenthesisOrMinus(string body, string offender)
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0106" }));
    ...
    public void SpacedMinusSignsGetThePlainMessageInstead()
    {
        ...
        Assert.That(IdsOf(diagnostics), Does.Contain("CM0101"));
        Assert.That(IdsOf(diagnostics), Does.Not.Contain("CM0006"));
    }
    ...
    public void AssigningToSomethingThatIsNotATargetIsReported()
    {
        ...
        Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0109" }));
    }
    ...
    public void AnIfExpressionWithNoElseIsOneDiagnostic()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(IdsOf(diagnostics), Is.EqualTo(new[] { "CM0112" }));
    ...
    public void ASemicolonInsideANestedIfExpressionReportsTheOuterOne()
    {
        ...
        Diagnostic missingElse = diagnostics.Sorted().First(d => d.Id == "CM0112");

        Assert.Multiple(() =>
        {
            Assert.That(missingElse.Span.Start.Line, Is.EqualTo(3), "the outer if, not the inner");
            Assert.That(IdsOf(diagnostics).Count(id => id == "CM0112"), Is.EqualTo(1));
        });
    }
    ...
    public void ATokenThatBeginsNoStatementIsOneDiagnostic(string body) =>
        Assert.That(IdsOf(ParseBody(body).Diagnostics), Is.EqualTo(new[] { "CM0107" }));
