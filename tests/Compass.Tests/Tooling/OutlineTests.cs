...
/// <summary>
/// <para>What <c>cm outline</c> tells an editor a file declares.</para>
...
public sealed class OutlineTests
{
    ...
    {
        string folder = Path.Combine(Path.GetTempPath(), $"compass-outline-{Guid.NewGuid():N}");
        Directory.CreateDirectory(folder);

        try
        {
            string path = Path.Combine(folder, "Outlined.cm");
            ...
            {
                Console.SetOut(said);
                code = Compass.Cli.Program.Run(["outline", path]);
            }
            finally
            {
                Console.SetOut(was);
    ...
    /// about Compass to draw the right icon — and would be the second place that knew it.</para>
    /// </summary>
    [Test]
    ...
    /// <para>One-based, as every position a reader sees in Compass is — an editor counting from
