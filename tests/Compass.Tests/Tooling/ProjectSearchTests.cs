...
namespace Compass.Tests.Tooling;

/// <summary>
/// <para>What <c>cm project</c> answers about which <c>.cmp</c> builds a file.</para>
...
public sealed class ProjectSearchTests
{
    ...
    private sealed class Tree : IDisposable
    {
        public Tree() =>
            Root = Directory.CreateDirectory(
                Path.Combine(Path.GetTempPath(), $"compass-project-{Guid.NewGuid():N}")).FullName;
    ...
    private static Answer Asked(string path)
    {
        ...
        {
            Console.SetOut(said);
            code = Compass.Cli.Program.Run(["project", path]);
        }
        finally
        {
            Console.SetOut(was);
    ...
    public void AProjectClaimsAFileItNames()
    {
        ...
        tree.Write("shop/shop.cmp", """
            project Shop
                source Program.cm
            end project
            """);

        string program = tree.Write("shop/Program.cm", AProgram);

        Answer answer = Asked(program);

        Assert.Multiple(() =>
        {
            Assert.That(answer.Project, Is.EqualTo(Path.Combine(tree.Root, "shop", "shop.cmp")));
    ...
    public void AFolderClaimsWhatIsDirectlyInsideItAndNoDeeper()
    {
        ...
        string inside = tree.Write("shop/models/Product.cm", AProgram);
        string below = tree.Write("shop/models/parts/Widget.cm", AProgram);
    ...
    public void AProjectClaimsWhatItReferences()
    {
        ...
        tree.Write("app/Program.cm", AProgram);

        tree.Write("core/core.cmp", """
            project Core
                source Tally.cm
            end project
            """);

        string tally = tree.Write("core/Tally.cm", "model Tally\nend model");

        // Asked from above, so the walk reaches app.cmp rather than core.cmp — the search starts
        // beside the file, and core.cmp is what claims it there. Naming the outer one is the
        // point: it is reached through the reference.
        Assert.That(
            Asked(tally).Project,
            Is.EqualTo(Path.Combine(tree.Root, "core", "core.cmp")),
            "the nearest project that claims it wins");

        string apart = tree.Write("elsewhere/Odd.cm", AProgram);
    ...
    public void HowManyWereReadSeparatesNoProjectFromNoProjectThatWantsIt()
    {
        ...
        string alone = tree.Write("alone/Program.cm", AProgram);

        Assert.That(Asked(alone).Searched, Is.Zero, "nothing above it is a project");

        tree.Write("taken/taken.cmp", """
            project Taken
                source Other.cm
            end project
            """);

        tree.Write("taken/Other.cm", AProgram);

        string unwanted = tree.Write("taken/Program.cm", AProgram);
    ...
    public void ASourceInsideACommentNamesNothing()
    {
        ...
        tree.Write("shop/Program.cm", AProgram);

        string draft = tree.Write("shop/Draft.cm", AProgram);
    ...
    public void ASourceAfterTheEndNamesNothing()
    {
        ...
        string stray = tree.Write("shop/Stray.cm", AProgram);
    ...
    public void AProjectAnswersWithItself()
    {
        ...
        string project = tree.Write("shop/shop.cmp", """
            project Shop
                source Program.cm
            end project
            """);
    ...
    public void TheNearestProjectThatClaimsItWins()
    {
        ...
        tree.Write("outer/outer.cmp", """
            project Outer
                reference inner/inner.cmp
            end project
            """);

        tree.Write("outer/inner/inner.cmp", """
            project Inner
                source Program.cm
            end project
            """);

        string program = tree.Write("outer/inner/Program.cm", AProgram);

        Assert.That(
            Asked(program).Project,
            Is.EqualTo(Path.Combine(tree.Root, "outer", "inner", "inner.cmp")));
    }
    ...
    public void ACircleOfReferencesIsNotFollowedForever()
    {
        ...
        tree.Write("one/one.cmp", """
            project One
                reference ../two/two.cmp
                source A.cm
            end project
            """);

        tree.Write("one/A.cm", AProgram);

        tree.Write("two/two.cmp", """
            project Two
                reference ../one/one.cmp
                source B.cm
            end project
            """);

        tree.Write("two/B.cm", "model B\nend model");
