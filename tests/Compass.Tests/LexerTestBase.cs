...
public abstract class LexerTestBase
{
    ...
    protected static string RepositoryRoot
    {
        get
        {
            ...
            while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "Compass.sln")))
            {
    ...
    protected static IEnumerable<string> SampleFiles =>
        new[] { "samples", Path.Combine("samples", "reference") }
            ...
            .SelectMany(folder => Directory.EnumerateFiles(folder, "*.cm"))
    ...
    public static IEnumerable<string> EverySampleFile =>
        Directory.EnumerateFiles(Path.Combine(RepositoryRoot, "samples"), "*.cm",
