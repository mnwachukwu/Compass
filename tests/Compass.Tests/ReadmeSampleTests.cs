...
/// grammar's tests in <c>Compass.Editors</c>, applied to the front page.</para>
/// </summary>
[TestFixture]
public sealed class ReadmeSampleTests : LexerTestBase
{
    ...
    [
        new TestCaseData("", "*.cm").SetName("the programs"),
        new TestCaseData("reference", "*.cm").SetName("the reference corpus"),
        new TestCaseData("negatives/compile", "*.cm").SetName("programs that do not compile"),
        new TestCaseData("negatives/runtime", "*.cm").SetName("programs that fail while running"),
        new TestCaseData("negatives/project", "*.cmp").SetName("projects that do not build"),
    ];
