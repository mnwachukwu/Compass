...
public sealed class GatheringAroundAFileTests : LexerTestBase
{
    ...
    /// <c>Book</c> is used in <c>Program.cm</c> and declared nowhere near it.</para>
    /// </summary>
    [Test]
    public void AFileInAProjectIsCompiledWithTheProject()
    {
        DiagnosticBag said = Checked(Sample("library", "Program.cm"));
    ...
    /// </para>
    /// <para>Opening the library's own <c>Book.cm</c> is the same program as opening the file
    ...
    {
        DiagnosticBag said = Checked(Sample("library", "books", "Book.cm"));
    ...
    {
        DiagnosticBag said = Checked(Sample("hello.cm"));
