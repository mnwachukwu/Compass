...
public sealed class RefusalMessageTests : LexerTestBase
{
    ...
    [
        ...
        ("integer zero = 0;\n        Console.WriteLine(1 / zero);", typeof(DivideByZeroException),
         "Cannot divide by zero. A divisor written down is refused while compiling as CM0324, "
        ...
        ("Date? none = Date.Parse(\"the fifteenth\");\n        Console.WriteLine(none.Value());",
         typeof(Compass.Runtime.EmptyOptionalException),
        ...
        ("Console.WriteLine(Program.Forever(1));", typeof(Compass.Runtime.RecursionTooDeepException),
        ...
        ("integer[] xs = {1, 2};\n        integer[] alias = xs;\n"
         + "        loop each x in xs\n            alias.Insert(9);\n        end loop",
         typeof(Compass.Runtime.SequenceChangedException), null!),
        ...
        //
        // Written down, this is CM0346 and never runs. Here it arrives in a variable, which is
        // the same division CM0324 draws around dividing by zero.
    ...
    /// no Compass program gets to it, which is a claim worth writing down rather than leaving
    ...
    private static readonly Dictionary<string, string> Unreachable = new(StringComparer.Ordinal)
    {
        ...
        ["an abstract function was reached with nothing written for it"] =
            "CM0241 refuses a model that can be constructed and leaves one open",
    ...
    private static IEnumerable<string> Sites()
    {
        string[] roots =
        [
            Path.Combine(RepositoryRoot, "src", "Compass.Interpreter"),
            Path.Combine(RepositoryRoot, "src", "Compass.Runtime"),
        ];
