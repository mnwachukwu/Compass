...
using Compass.Cli.Debugging;

namespace Compass.Tests.Debugging;
...
public sealed class DebugAdapterTests
{
    ...
    /// <summary>Runs a session over a one-file program named <c>Program.cm</c>.</summary>
    private static List<JsonObject> Session(string program, params string[] requests) =>
        SessionOver([new Written("Program.cm", program)], requests);
    ...
    {
        string folder = Path.Combine(Path.GetTempPath(), $"compass-debug-{Guid.NewGuid():N}");
    ...
    public void LaunchingSomethingThatWillNotCompileIsRefused()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That((string?)answer["message"], Does.Contain("CM0404"));
        });
    }
    ...
    public void RefusalsAreToldApartByWhereTheyAre()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(lines[0], Does.Contain("Program.cm(").And.Contain("CM0201"),
    ...
    private static readonly Written[] TwoFiles =
        [new Written("Program.cm", Calling), new Written("Numbers.cm", Numbers)];
    ...
    /// <c>cm run</c> compiles — the program named, and the shared code beside it.</para>
    /// </summary>
    [Test]
    public void AFrameSaysWhichFileItIsIn()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                ...
                Is.EqualTo(new[] { "Numbers.cm", "Program.cm" }),
                "each frame names its own file, not the one the program started in");

            Assert.That(
                (string?)frames[0]!["source"]!["path"],
                Does.EndWith("Numbers.cm").And.Contains(Path.DirectorySeparatorChar),
    ...
    public void ABreakpointBelongsToTheFileItWasSetIn()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That((string?)frames[0]!["source"]!["name"], Is.EqualTo("Numbers.cm"),
