...
public sealed class ReleaseAssetTests : LexerTestBase
{
    ...
    /// without one — `compass-osx.tar.gz` — cannot say which of two Macs it is for, and the
    ...
    [
        ("win-x64", "compass-win-x64.zip"),
        ("win-arm64", "compass-win-arm64.zip"),
        ("linux-x64", "compass-linux-x64.tar.gz"),
        ("linux-arm64", "compass-linux-arm64.tar.gz"),
        ("osx-x64", "compass-osx-x64.tar.gz"),
        ("osx-arm64", "compass-osx-arm64.tar.gz"),
    ];
    ...
    {
        string[] found = [.. Regex.Matches(Workflow, @"compass-[a-z0-9-]+\.(?:zip|tar\.gz)")
